...
8. **Recently-shipped-feature regression pattern.** When the regression was introduced by a feature shipped on the *current* branch (not inherited from `main`), the pattern has two distinct sub-paths with fundamentally different remediations. Discriminate up front by asking: are the failing tests *tests of the new feature itself* (its own goldens are red), or are they *downstream test/fixture calibrations in unrelated scenarios* that incidentally exercise the feature's surface (the feature's own goldens stay green)? Pick the sub-path before evaluating any remediation.
