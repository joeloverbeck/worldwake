...
- Section 6: End-State Inventory & Resources (agent possessions, place contents). Place contents for locations with entity pollution (500+ SocialArtifacts from post_notice/tell spam) may appear as extremely long single lines. If a place entry is dominated by SocialArtifact entities, note the count and skip individual enumeration — the pollution pattern itself is the diagnostic signal, not the specific entity IDs.
...
9. **Social isolation** -- Check location tracking: if agents are co-located for extended periods (20+ ticks) with no Tell, AskWitness, or Trade actions between them, flag it. Social isolation is not binary — even with tell actions present, flag if: (a) no Trade actions despite co-location with agents having complementary needs/inventory, (b) social actions are heavily unidirectional (one agent doing most telling), (c) role-specific social actions are unused (merchant never staffs market, leader never declares policy), or (d) tell actions produce SocialArtifacts but no observable change in recipient behavior (information deadweight).
...
- **Sleep+relieve behavioral collapse**: Agent's action repertoire narrows to only sleep and relieve_wilderness for 500+ ticks. Not a degenerate plan loop (no GoalSatisfied[steps=0]) and not obligation spam (no obligation action dominates). Instead, all non-trivial goals (AcquireCommodity, Wash, social goals) fail planning (budget-exhausted or frontier-exhausted) or lack local affordances, so the agent defaults to the only goals with 1-step local plans. Signature: Section 2 behavioral transition shows narrowing to 2 types (sleep + relieve), Section 4 confirms the pattern persists for remaining ticks, Section 7 shows budget-exhausted for acquisition goals. Cross-reference with smell 10 — often caused by geographic food desert (agent at a location with no food production affordances).
...
## Trend Comparison
[Include this section only if a prior `simulation-observer-report.md` exists in git history for the same scenario and seed. Use `git log --oneline -- reports/simulation-observer-report.md` to check, then `git show HEAD~N:reports/simulation-observer-report.md` to read the prior version.]

| Smell | Prior Severity | Current Severity | Delta |
|-------|---------------|-----------------|-------|
| ... | ... | ... | improved / worsened / unchanged |

[If no prior report exists for this scenario+seed, omit this section entirely.]
