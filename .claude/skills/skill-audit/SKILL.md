...
An optional free-text directive may follow the path (a focus area, an exclusion, or a known-context note — e.g., `"don't propose adding references to specific tickets"`). When present, honor it as a constraint on the findings and suggestions, and surface it explicitly in the report's Alignment Check as a scope note so its influence on the audit is auditable.
