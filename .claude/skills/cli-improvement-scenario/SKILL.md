...
If the error can be attributed to a specific commit (e.g., from `git log`), run `git diff <commit>~1 <commit> -- crates/worldwake-core/src/` to identify all profile struct changes in that commit. Note all affected structs — RON parsing stops at the first error, so additional missing fields in other structs won't surface until earlier ones are fixed. Fix all of them together in Step 4 to avoid repeated fix-validate cycles.
