...
The dump contains 7 sections (some versions add Section 8):
...
- **Section 3**: Anomaly Flags — mechanically detected oddities. **For narrative extraction, treat these as candidate inflection ticks worth investigating, not as defects to flag.**
- **Section 4**: Raw Event Sample — first 100 + last 100 events.
- **Section 5**: Per-Agent Belief Summary — known entities, believed locations, social/told/heard/institutional beliefs (item type names like "Apple", not EntityIds).
- **Section 6**: End-State Inventory & Resources — agent possessions, place contents.
- **Section 7**: Per-Agent Decision Summary — planning outcomes, goal selection, committed actions, failed plan attempts, blocked desires, affordances. **This is the spine of the per-agent narrative pass.**
- **Section 8** (optional): Budget Exhaustion Snapshots.
...
1. **Build the EntityId → name map first** from Section 1's agents and places tables. Use names everywhere in the report; translate EntityIds when quoting raw rows. Item EntityIds in Section 7 are not in the map — leave them as `eXgY` references when quoting.
...
3. **Section 5 is read for evolution, not just contents**. The report needs *changes* in beliefs (new entity learned at tick X, hearsay accepted at tick Y, contradiction registered at tick Z), not just final-state belief lists. If the dump's belief summary is end-state-only, see `traceability-fix-protocol.md` — this is a frequent cheap-fix candidate.

4. **Section 7 is dense**. Individual rows can exceed 5000 tokens. Never use the Read tool with `limit > 10` on Section 7. Instead:
...
5. **Cross-section anchoring**. Every claim in the per-agent narrative must be traceable to either Section 2 (location/action timeline), Section 5 (belief), Section 6 (final inventory), or Section 7 (decisions). If you cannot anchor a claim, drop it or trigger the traceability protocol.

## ControlSource Heuristic

The dump does not list `ControlSource` per agent. If the scenario `.ron` is accessible (it is, during the pre-flight scan), read `AgentDef.control_source` directly. A human-controlled agent with no input will have minimal Section 7 planning activity — narrate this honestly as "no AI decisions to recount" rather than as agent failure.

## When Section 8 Is Present
