...
<omit this heading entirely if sub-check (b) found no serialization impact at all. If serialization changed in a single ticket only (one bump, no cascade), omit this heading but include a one-line merge note in that ticket's body explaining the bump and any deliberate avoidance of additional bumps in sibling tickets — e.g., `Merge note: Ticket 001 bumps SAVE_FORMAT_VERSION 53→54; Ticket 002 deliberately avoids a second bump via #[serde(default)] on the new field.` Place the inline merge note as the final block in the ticket body, below the Test Plan section, on its own line prefixed `Merge note:` (no heading) — this keeps it visible to merge-coordinator readers without disrupting the template's standard section sequence. If multiple tickets bump `SAVE_FORMAT_VERSION`, keep this heading and list the per-ticket bump sequence as a merge-order constraint note — e.g., `SAVE_FORMAT_VERSION cascade: 002→49, 003→50, 004→51, 005→52, 006→53 — must land in dependency order; landing two tickets out of order produces a value collision.` Other cascading constraints (schema versions, magic numbers in shared registries) follow the same format.>
```
