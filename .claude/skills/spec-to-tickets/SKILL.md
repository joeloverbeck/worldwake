...
- **Split deliverables**: when a single deliverable cleanly partitions across multiple tickets (e.g., per-category, per-crate, per-domain) and each ticket fully owns its slice, mark it "split" rather than "distributed." Split differs from distributed in ownership: split deliverables have a per-ticket owner with a partitioned scope (each ticket fully implements its slice); distributed deliverables have no dedicated owner per ticket (each ticket implicitly contributes to a shared concept). In the Step 4 Scope column, name the slice in the primary scope (e.g., `Scope: D5 — combat.rs (7 actions)`). In the Step 6 coverage mapping, render as `D5 → 003+004+...+009 (split — partitioned by <axis>)` — `+`-joined IDs with a parenthetical naming the partition axis (file, crate, action category, domain). The split classification preserves the per-ticket owner relationship that distributed loses, which matters for the downstream `/implement-ticket` flow where each ticket reads its slice as its primary scope.
