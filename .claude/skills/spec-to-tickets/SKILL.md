...
<only when the spec contains an "Authoritative-to-AI Impact Analysis" section (typically added by `/reassess-spec` when a spec modifies action preconditions, validation functions, affordance generation, candidate emission, or goal satisfaction). Map each of the 7 checklist points to the ticket(s) that address it. Use this format, one line per point:>
