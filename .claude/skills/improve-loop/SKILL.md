...
Some campaigns mutate files (e.g., configuration, data definitions) whose compiled output is captured in snapshot/golden test fixtures. When the harness runs tests, these fixtures fail — not because the code is broken, but because the fixtures are stale.
...
- **Tier 1** (primary target): Configuration or data changes — committed on ACCEPT, rolled back on REJECT
- **Tier 2** (infrastructure): Extensions that enable Tier 1 — committed separately, may persist even if Tier 1 REJECT
...
4. If the project requires dependency installation or build setup in the worktree, run that before proceeding (e.g., `cd $WT && cargo build` for Rust projects, `cd $WT && pnpm install` for JS projects).
...
- If strategy is `backtrack`:
  ...
  2. Select the checkpoint with the best metric (using Metric Direction Comparison Helpers below). If metrics are within 1%, prefer the one with lower `lines_delta_cumulative`.
...
- **Scope check**: Verify that ONLY declared mutable files were modified (`git diff --name-only` against mutable file list from program.md). When tiered mutability is active, validate against the UNION of all tier file lists. If any non-mutable file was changed:
...
- Count `lines_delta` for this change (net lines added minus lines removed across mutable files). When tiered mutability is active, count only Tier 1 lines delta for the accept/reject decision.
...
**Suspicion gate:**
- Compute `improvement_pct` using the Metric Direction Comparison Helpers.
...
- Compute improvement: `improvement_pct` using the Metric Direction Comparison Helpers.
...
For single-tier campaigns (default):
```bash
cd $WT && git add <changed-files> && git commit -m "improve-loop: <description> (<PRIMARY_METRIC_KEY>: <old_metric> -> <new_metric>)"
```

For tiered campaigns: commit according to the tiered commit protocol defined above.
...
For tiered campaigns: follow the tiered rollback protocol (Tier 1 rolls back; Tier 2 evaluated independently; Tier 3 always commits if tests pass).
...
- **Never stop the loop** — run until externally interrupted, `MAX_ITERATIONS` reached, or ceiling detected.
...
- **Never modify safety-critical config during meta-review** (see Step 1g forbidden list).
...
- **Always respect program.md custom accept/reject logic** — when program.md defines its own conditions, they are authoritative over skill defaults.
