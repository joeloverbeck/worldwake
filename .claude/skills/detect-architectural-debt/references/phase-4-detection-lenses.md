...
6. **Rust false-positive warnings**: Include these explicit instructions in every sub-agent prompt:
   - Do NOT count trait implementations across different structs as duplicated logic — each `impl` of a trait (e.g., `GoalBeliefView`, `Display`) serves a different data source or purpose. Only flag duplication when the same free function or inherent method body is copy-pasted across files.
   - Count enum variants at the `pub enum` definition site only. Do not estimate variant count from grep-counting `EnumName::VariantName` references — references include match arms, constructors, and type annotations that inflate the count.
   - Distinguish type-parameter usage of an enum or struct (e.g., `CommodityKind` as a field type or function argument) from logic-branching usage (match arms with variant-specific behavior). High reference count with no duplicated branching logic is a well-used domain type, not scattered logic.
7. **Confidence tagging**: Ask the agent to tag each finding with a confidence level: **High** — verified by reading the function body or definition site; **Medium** — inferred from grep patterns and naming similarity; **Low** — inferred from file counts or reference counts only. Phase 4.5 verification prioritizes Low and Medium-confidence findings.
8. **Thoroughness and output format**: Request "very thorough" exploration. Ask for file paths, line numbers, and counts for each signal found.
