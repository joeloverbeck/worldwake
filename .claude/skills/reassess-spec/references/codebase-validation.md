...
- **5c. Default and constructors**: For field additions, check `Default` impl and builder/constructor functions. For field additions to components that are deserialized from scenario files (RON), verify the field has a serde default (`#[serde(default)]` or `#[serde(default = "...")]`) so existing scenarios continue to deserialize. Grep existing scenarios for the component name to confirm whether scenario deserialization is a concern.
