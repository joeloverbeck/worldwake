...
**Replaced-and-split case**: When an original deliverable's mechanism is *replaced* AND the replacement *spawns a sibling* introducing new mechanism (a single original D-section becomes a re-mechanism'd parent plus a net-new-mechanism child), count it as **1 redesign (the replaced parent) + 1 addition (the new-mechanism sibling)** — do not double-count the parent as both a redesign and an addition, and do not collapse the sibling into the parent's redesign. Anchoring example: original `D2 (verification synthesis)` is replaced by a different mechanism (a step constructor) *and* split off a net-new `D3 (seam-side candidate construction)` whose construction site and call surface the original parent never touched — that is `N_r += 1` (old D2's mechanism changed) and `N_a += 1` (D3 is net-new mechanism), not `N_r += 2` or `N_a += 2`.
...
- **Discrete options (2-4), single question**: Use `AskUserQuestion` with a recommended default — but only when the standalone question is a preference or factual pick whose option set is genuinely closed. When the single standalone question is a design/architecture/FOUNDATIONS-tradeoff decision (one where "you decide based on FOUNDATIONS" is a plausible answer), prefer inline plain-text bullets even though it stands alone: `AskUserQuestion`'s discrete-option UI cannot represent delegation-to-principle as a first-class choice, so the user is forced to reject the tool to express it, whereas inline plain-text accommodates both a discrete pick and a delegation in one response with no tool round-trip.
