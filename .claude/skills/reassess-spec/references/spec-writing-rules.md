...
Grep the updated spec for: (1) eliminated stale references (should return zero matches), (2) corrected references (should return expected matches), (3) file path references in newly added deliverables (should resolve to existing files), and (4) if the spec contains a Decomposition Hint section, grep it for the deliverable surfaces (function names, trait names, key types) named in each ticket bundle and confirm they match the current D-section text — mismatches mean the ticket boundaries describe stale concepts and will mislead `/spec-to-tickets` downstream. Record results for Step 8.
