...
In plan mode, Pre-Apply Verification (below Step 6) runs after ExitPlanMode approval and before Step 7 spec writing. It confirms that approved findings still hold (e.g., no codebase changes occurred between approval and writing).

## Process

Follow these steps in order. Do not skip any step.

**Spec type classification**: Before beginning Steps 2-3, classify the spec as:
- **(a) New system** — introduces new components, actions, goal kinds, or information paths. Full checklist applies (all Step 3 sub-steps, full Step 4, Section H required).
- **(b) System extension** — extends existing components, actions, or enums without introducing new systems. Steps 3.5, 3.6, 4.4 apply; Section H updates required only for new deliverable sections.
- **(c) Structural refactor** — trait/module restructuring, layout changes, or code organization with no behavioral changes. Skip Steps 3.5, 3.9, 4.4; Section H is N/A. Focus validation on symbol existence, count accuracy, and blast radius.

This classification reduces unnecessary work for refactoring specs while ensuring full rigor for system specs. If uncertain, default to the more rigorous classification.
...
**Self-authored spec note**: If the spec was authored earlier in this session, full validation (Steps 2-3) is still required — the authoring phase may have introduced unchecked assumptions that only systematic codebase validation catches.
...
2. **Types and interfaces**: Grep for each type name. Confirm it exists, check its current shape (fields, members). If the spec assumes a field that does not exist or has a different name/type, record the discrepancy. For types the spec uses in formulas or struct definitions (not just extends), verify that the assumed field types match the actual types. Pay particular attention to numeric types (`u32` vs `Permille` vs `i32`) — the spec may assume a different numeric representation than what exists. If a spec formula or comparison combines values of different numeric types (e.g., `Permille` compared with `u32`), flag as a LOW Improvement noting the implementation-time conversion. This helps implementers anticipate type bridging without requiring the spec to spell out language-level casts. If the spec proposes serializing a type (bincode, serde, etc.), verify it derives `Serialize`/`Deserialize`. If the spec references hash functions in acceptance criteria, verify the functions exist and that the hash input includes/excludes the expected data. For field additions to non-ECS structs (e.g., belief-layer types, snapshot types), check serde derives, `#[serde(default)]` for backward compatibility, Default impl impact, and whether derivation/construction functions (e.g., `derive_entity_summary()`) can populate the new field from their inputs. If the derivation function reconstructs the struct from a different data source (e.g., claims) that lacks the new field, flag the propagation gap as an Issue and note what fallback is needed (e.g., lookup from prior state).
3. **Functions and methods**: Grep for each function. Confirm signature, module location, and export status. Note any signature differences from what the spec assumes. For new functions the spec proposes, validate that the signature's parameters provide sufficient data at every intended call site. If a parameter type doesn't carry enough information (e.g., the function needs belief context but the proposed parameter is a payload-only type), flag it as an Issue. For existing functions the spec proposes to modify (adding code at specific lines), verify that the function's parameters and local scope include the variables the proposed code references. If a proposed code change uses a variable that is not in scope at that location, flag as an Issue and note what signature changes are needed to thread the variable through. For specs that propose categorization or partitioning of existing symbols (e.g., splitting a trait's methods into sub-traits, distributing enum variants across sub-enums), verify the partition is complete (all symbols accounted for) and disjoint (no symbol in two categories). Verify that stated counts match the number of listed names. Automated scripts may be needed for large sets (>20 symbols).
...
5. **Component fields and ECS registrations**: If the spec does not propose adding fields to existing components, creating new components, or extending discriminator enums, skip sub-steps 5a-5g. Otherwise, validate component definitions and check for conflicts, following these sub-steps:
   ...
   - **5d. Downstream consumers**: For field type changes or removals, perform full downstream consumer analysis (sub-step 6 below).
...
6. **Downstream consumers**: For types or interfaces the spec proposes to modify, grep for all import sites and usage points. Record the blast radius — files that would need updating. When the spec adds new variants to an existing enum (whether ECS-related or not):
   - **Trait bounds**: Check the enum's derive macros and trait bounds. Verify the new variant's fields satisfy all existing bounds (e.g., `Serialize`, `Eq`, `Clone`). If the enum has `#[allow(clippy::large_enum_variant)]`, note whether the new variant changes size characteristics.
   - **Exhaustive match analysis**: Grep for pattern matches on existing variants (e.g., `EnumName::ExistingVariant`) to identify all match sites that will need a new arm or wildcard. This catches files that import the type but pattern-match on specific variants rather than the type name itself. Especially important for enums matched across multiple crates.
...
9. **Behavioral claim validation**: For each claim the spec makes about who reads, writes, or depends on a type/field at runtime (e.g., "only test code reads `state_deltas`", "no system queries X"), grep for all call sites and classify each as runtime vs. test-only (`#[cfg(test)]`). If any runtime caller contradicts the claim, flag as CRITICAL. If the claim is technically wrong but the design is still safe (e.g., the runtime caller only reads current-tick data that is never compacted), note both the correction and the safety argument.

For specs with many references (>10), consider launching parallel Explore agents organized by theme (e.g., one for action/type references, one for AI/test references, one for dependencies and infrastructure). This is more efficient than sequential validation. Choose themes based on the spec's scope — the split should minimize cross-agent dependencies. Examples: one for action/type references, one for AI/test references, one for dependencies and infrastructure. After agent results arrive, cross-reference their findings against the spec's type assumptions and formulas. Agents validate existence; you must validate semantic compatibility (e.g., the spec says Permille but the codebase uses u32). For specs that propose static lookup tables indexed by a dispatch key or discriminator enum, verify that the key's granularity matches the lookup's discrimination needs. If the spec assumes per-payload-variant behavior but the key collapses payload variants into a single entry, flag as an Issue. Spot-check agent claims about existence/registration with direct Grep or Read before including them in findings. Agent results are approximate — treat them as leads, not facts. In plan mode, Explore agents are inherently compatible (read-only exploration) — no special handling needed. For structural refactor specs (type c), direct agents toward *discrepancy checking* (do claimed counts match? do named symbols exist? what is the actual blast radius?) rather than broad exploration (what does this module do?). The more specific the agent prompt, the more actionable the result.
...
Before writing changes in Step 7, verify each finding one more time with targeted checks (e.g., a script that counts actual vs. claimed values, a grep confirming a symbol's presence or absence). If a finding is invalidated by this verification, re-present the corrected finding to the user before applying. Do not silently substitute different changes than what was approved.
...
- Items excluded by reassessment-driven scope changes (distinct from user-deferred items) — note why. If none, omit this bullet.
