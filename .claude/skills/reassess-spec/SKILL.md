...
**Hybrid specs**: Apply the union of applicable steps — use the most rigorous classification's checklist for shared steps. Three-way and four-way hybrids compose the union of the named two-way rules; when constituent rules conflict on a shared step, apply the most rigorous per the leader sentence above. Common hybrids:
  ...
  - **(c) + (b)-tooling-only** (structural refactor of test layout or build artifacts paired with tooling-script rewrites — e.g., consolidating per-file test binaries while updating the inventory script and downstream docs/skills): Full (c) checklist for structural changes (file moves, layout refactor, naming-convention shifts). (b)-tooling-only's 3.1–3.4 + 3.8 for the script rewrites; skip 3.5–3.7 unless the spec extends cross-crate types. Skip 3.9. 4.4 N/A. Section H is N/A — preserve if the spec authored one. Step 3.6 limited to the tooling binary's consumers (e.g., `docs/generated/`, sibling scripts, downstream skill files).
...
Step-level `TaskCreate` (one task per Step 4 / Steps 5–6 / Step 7 / Step 8 of this skill) is **recommended** when the reassessment dispatches 3+ parallel Explore agents OR when a single Step 6 question round bundles 3+ questions. For single-agent reassessments and for auto-mode reassessments with no open questions, step-level tracking is optional. This is independent of the per-reference checklist use and may coexist with it.

The harness may auto-fire generic "task tools haven't been used recently" reminders independent of this skill's thresholds; treat them as advisory — the per-reference (>15) and step-level (3+ agents OR 3+ bundled questions) rules above are authoritative. Below those thresholds, mental tracking is the intended path and the reminders should not trigger task scaffolding.
...
**Quantitative-baseline carve-out** — When the spec's classification is (c) structural refactor or (b)-tooling-only (including the (c) + (b)-tooling-only hybrid), purely quantitative baseline checks may precede the reference reads: counting files matching a glob, confirming directory existence, enumerating top-level test entries, or other path-and-count probes whose interpretation does not depend on the pattern-specific triggers. The content-tied acknowledgments for both reference files must still appear in chat before any pattern-application work begins (3.5 component checks, 3.6 cross-crate consumer analysis, pattern-trigger checklists from `worldwake-validation-patterns.md`, etc.). This carve-out exists because structural-refactor and tooling-only specs are dominated by counting and path-existence claims that the patterns reference does not shape; it does not extend to (a), (e), or any classification that touches simulation types.
...
- **Scope-extending mismatch**: the approved recommendation still applies, but fulfilling it requires adding a new deliverable, migration, subsystem change, or invariant-driven removal of adjacent existing state, none of which were discussed at question time. The recommendation was not refuted and the evidence was not merely refined — the scope grew. A common form is fabrication-confirmation triggering an FND-28 removal: a finding confirms a spec-claimed "existing field" was fabricated, the reframed recommendation adds the correctly-shaped new field, AND the spec body must also retire a legitimate-but-incomplete adjacent field per FND-28 (no shim coexistence with the new field). The retirement is consumer-migration scope the user only sees in the post-implementation spec body unless flagged at the pre-apply gate. Note the scope extension inline in the Result column of the pre-apply table (e.g., "scope-extending: requires new D4 to relocate `MaterializationTag` from sim to core so the payload type can live in core — recommendation unchanged") and proceed. Additionally, surface the scope extension in the Step 8 summary under a dedicated line (or in the classification-shift note) so the user sees it in both the pre-apply table and the final summary. If the scope extension constitutes a cross-crate type migration, also apply the Pre-Process "Emergent migration at Step 7" guidance and run 3.6 cross-crate consumer analysis before finalizing edits.
