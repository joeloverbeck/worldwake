...
- If the brainstorm is a **triage producing ≥ 2 deliverables of any combined type (specs + tickets) derived from a named report, scenario run, or remediation wave**, additionally write a short `docs/triage/YYYY-MM-DD-<topic>-triage.md` summarizing: the source report/scenario/finding set, accepted items (with the full path to each written spec or ticket and a one-line rationale), dismissed items (with a one-line reason each), and any follow-up work items the triage identified but did not action. Keep the triage file under 80 lines. Do not duplicate spec or ticket content — reference by path. The triage file makes the brainstorm's decisions durable and shareable without re-running the brainstorm. For triage brainstorms producing only one deliverable, or for triages whose deliverables are routine work not derived from a named source, skip this file — the individual deliverables and the `IMPLEMENTATION-ORDER.md` update are sufficient history.
