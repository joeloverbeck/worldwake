...
Announce classification to user
         |
         |--→ (triage brainstorm) verify claims → triage verdict + scoping forks → skip to "Write design doc"
         v
...
7. **Project context**: Explore existing implementations relevant to the topic before starting the interview — this context informs better questions. For tooling/process topics, examine existing instances of the thing being designed (e.g., existing skills, configs, workflows — their structure, size, patterns). For skills, also check the on-disk layout of sibling skills — conventional subdirectories like `references/` and `scripts/` indicate the expected multi-file pattern. For codebase topics, check relevant files, specs, and tickets. Launch Explore agents for broad surveys when needed. Keep exploration targeted to what informs the interview. For brainstorms triggered by frustration or repeated failure, also explore the history of attempted fixes — not just the current state. Patterns of repeated remediation specs, escalating complexity, or accumulated workarounds are signals that the approaches (Step 3) should include radical options (reset, strip, rebuild from scratch) alongside incremental fixes. A long chain of narrowly-scoped fixes that didn't resolve the underlying problem is evidence that the problem may be structural, not tactical. For triage brainstorms whose question concerns the architectural quality of recent work (commits, tickets, PRs, refactors), read the actual diffs (`git show <commit>`, file-level Reads) — do not rely on ticket descriptions alone. Implementer claims about root-cause fidelity are hypotheses to verify, not established facts. For refactor brainstorms (renaming, moving, restructuring existing assets), explicitly enumerate consumers — what scripts, docs, tests, generated artifacts, sibling skills, or developer workflows reference the asset's current location, name, or structure? Grep for the asset's canonical patterns (filenames, paths, identifiers) and read the top hits to understand how each consumer is coupled. Use the resulting consumer inventory to scope approaches realistically before Step 3, so the initial proposal reflects true blast radius rather than asset-only complexity. Without this, the first approach proposal underestimates scope; the user often has to prompt for a blast-radius reassessment that triggers a full re-scoping round.
...
1. **One question per message.** Never ask multiple questions at once. Carve-out: multi-axis recommendation batching is permitted in the closing turn(s) of an interview when (a) each axis is small/operational, (b) your recommendation is stated for each axis, and (c) the user's only decision is which (if any) to flip. This counts as one question for purposes of this rule. Do not use this carve-out for motivational, scoping, or design-shaping gaps. (Exception: in the triage context, the Step 3 triage carve-out permits batching a few scoping forks into a single `AskUserQuestion` — see there.)
...
**For triage/analysis brainstorms** where the deliverable is a set of work items (tickets, specs) derived from evaluating a report or findings, the "approaches" step may be replaced by presenting the triage recommendation: which items warrant action, which are dismissed, and why. The user's approval of the triage recommendation serves the same gating purpose as choosing an approach. When the triage leaves independent scoping forks that change the deliverable set (e.g., strip-vs-build, defer-vs-include, fold-vs-separate), batch up to 3 into a single `AskUserQuestion`, each with a stated recommendation. This is the triage analogue of the closing-turn batching carve-out and overrides Interview Rule 1's scoping exclusion for the triage context. On the triage path, the triage verdict — together with its per-item evidence/rationale — stands in for the interview→approaches confidence-block transition; an explicit confidence percentage is optional here, since the evidence table itself demonstrates the analysis. **Verify before triaging.** When the triage evaluates an external report's claims, confirm each load-bearing claim against current code *before* judging benefit — dispatch parallel Explore agents for independent claim clusters and report CONFIRMED / REFUTED / PARTIAL per claim. Triage benefit, not correctness, only after the factual base holds. **Dedup against existing work.** For each item you intend to accept, grep `specs/` and `archive/specs/` / `archive/tickets/` for the surface it touches and classify it as **dismissed** (already addressed), **reaffirmed** (covered by existing pending work — name the path), or **accepted** (genuinely new). An item warrants a new spec only after confirming no completed or pending spec/ticket already owns it; this is the discipline that prevents re-litigating decisions a prior iteration already made. When the dedup work is delegated to a subagent, prefer the per-item "what's left" evidence over any summary verdict the agent appends. Subagent conclusion tables routinely overreach beyond what the cited evidence supports — independently classify each item from the cited per-spec paths plus the "what's left" notes, not from the agent's summary line. **Excluded-per-directive.** A fourth disposition applies when the user's invocation carves specific items out of scope (e.g., "don't include specs X–Y; they'll be absorbed later"). Record these distinctly from dismissed and reaffirmed — they are neither rejected on merit nor already-covered by pending work; the reason is a user instruction. Name the directive and the excluded items in both the triage doc and an "Excluded This Iteration (per user directive)" note in `IMPLEMENTATION-ORDER.md`, pointing each to the spec/ticket that will later absorb it. **Stop-condition / no-proposal audits.** When the source is a stop-condition audit (the source recommends stopping a cycle or doing no new work), the triage shape is *critically reassess the recommendation for each watchlist/deferred item*. Each watchlist item gets an accept/concur verdict; an accept means writing a spec for it despite the source's defer recommendation. The dismissal record is then primarily a record of where the critical reassessment concurred with the source rather than where it rejected source proposals.

When triage produces **multiple deliverables** (N specs, N tickets, or a mix), the single triage approval gates all N — do not re-prompt for approval per deliverable. Each deliverable still independently satisfies its own format requirements (spec-drafting-rules.md for specs, `tickets/_TEMPLATE.md` for tickets), but no per-item user approval is required. The Step 6 Next Steps menu is presented once per triage, summarizing all N produced deliverables. For spec deliverables, `specs/IMPLEMENTATION-ORDER.md` gets a single update encompassing all N specs (typically a new Adjunct Wave — see Step 5 spec-deliverable branch). **Spec vs. ticket routing.** Route an accepted item to a **spec** when it has cross-system reach, introduces or changes a contract, or needs golden + FND-31 proof; route to a **ticket** when it is a bounded fix to existing code within one module. When unsure, prefer a spec — it can decompose into tickets later.
...
**Triage path**: When Step 3 was resolved via the triage carve-out (verdict + scoping forks in a single `AskUserQuestion`), skip this entire step — there is no design to present section by section. The accepted specs/tickets are the design, already gated by the single triage approval. Proceed directly to Step 5's triage branch and write the deliverables.
...
**Deliverable type lookup**: Find your deliverable type in the table below, then jump to the matching detailed bullet for full guidance.

| Deliverable type | Target path | Skip `docs/plans/` design doc? |
|------------------|-------------|--------------------------------|
| Skill design (new) | `.claude/skills/<name>/SKILL.md` | Yes — skill file IS the design |
| Modifying or reconciling existing skills | The modified skill file(s) | Yes — edits ARE the design |
| Modifying existing hand-authored doc | The target doc | Yes — edits ARE the design |
| Replacing an existing artifact | Replacement + cross-ref check | Per replacement plan rules |
| System spec | `specs/<Name>.md` | Yes — spec IS the design |
| Triage producing ≥ 2 deliverables | Specs/tickets + `docs/triage/YYYY-MM-DD-<topic>-triage.md` | Yes |
| Hybrid (e.g., code + spec) | Plan file + spec; multi-deliverable plan | No — plan file describes sequence |
| Data-gathering required | Plan file with pre-deliverable phase | No — plan file is the design |
| Enhanced diagnostics / investigative tooling | Plan file with tooling + fixes | No — plan IS the design |
| Implementation tickets | `tickets/<id>.md` | Yes — tickets ARE the design |
| Scenario file with golden tests | Plan file describing scenario | No — plan IS the design |
| Dev-tooling crate / workspace member | `docs/plans/YYYY-MM-DD-<topic>-design.md` | No — design doc is the deliverable |
...
- If the brainstorm topic produces a system spec (architectural change, new subsystem, planner modification, or any change requiring formal spec-drafting-rules compliance), the deliverable is the spec file in `specs/`. Skip the `docs/plans/` design doc — the spec IS the design. **Filename prefix**: Before writing, grep `specs/` and `archive/specs/` for the current spec-prefix scheme. If the project uses a single consistent series (e.g., `S###` for everything including tooling/observer specs), continue that sequence. Only introduce a new prefix after user approval in the design presentation. Read `docs/spec-drafting-rules.md` and ensure the spec includes all mandatory Section H analyses: (1) information-path, (2) positive-feedback, (3) concrete dampeners, (4) stored-state vs. derived read-model list. Also verify CLAUDE.md mandatory spec requirements: Permille for all [0,1] or [0,1000] range values, profile-driven parameters (no hardcoded numeric constants), SystemFn Integration section, Component Registration section, cross-system interactions documented via FND-26. Also read a recently-completed spec from `archive/specs/` matching the deliverable's domain (e.g., an AI-architecture spec for AI work, a gameplay spec for gameplay work) to confirm conventional section ordering, problem-statement tone, and citation style — drafting-rules names the required sections but precedent names the conventional sequence. Scale how much you read by spec type: `head -80` is enough for light specs (correctness-fix, trace-enrichment, tooling-boundary) where the structure is front-loaded, but for **full simulation system specs** (those carrying the FND-01 Section H 18-point causal-hooks declaration plus FND-31 scenario validation), `head -80` undershoots — the Section H declaration, the stored-state-vs-derived table, the planner-formalism and belief-view source-class tables, and the scenario-validation section are the structural bulk and live well past line 80 (often line 300+). Read those sections of the precedent in full, since they are the part most prone to convention drift. **Correctness-fix / pure-refactor / trace-enrichment specs**: For specs that change simulation behavior but introduce no new state, systems, components, or feedback loops, or specs that surface new diagnostic/trace fields over existing authoritative state without changing simulation behavior, do not use the tooling-boundary opt-out table; instead include each mandatory Section H header with "Not applicable" and a one-line reason (Information-Path, Positive-Feedback, and Concrete Dampeners are typically N/A; the Stored-State vs. Derived Read-Model List still applies and must declare whether any new types are authoritative or derived). **Tooling-boundary specs**: If the spec is a tooling-boundary spec (dev tool, debugger, diagnostic, visualizer — observer pattern per FND-29 introducing no new simulation state, laws, components, actions, systems, or feedback loops), the simulation-system requirements in `docs/spec-drafting-rules.md` do not apply. Instead include an "Explicit Opt-Outs" table listing each rule (Section H causal-hooks, Permille, profile-driven parameters, SystemFn integration, component registration, cross-system interactions via FND-26) marked N/A with one-line reasoning per rule. Adjust the Step 6 menu accordingly — omit the "create a spec" option and use this template:
  ...
  The menu is presented once per triage, not once per spec. After writing the spec(s), update `specs/IMPLEMENTATION-ORDER.md`. For specs derived from a specific report, scenario run, or triage session, create a new `### Adjunct Wave: <topic>` section in the appropriate Phase, with a one-paragraph derivation preamble stating the source report/scenario, its key findings, and which proposals were accepted vs. dismissed. Place the spec list as bullets under the wave heading with dependency status. If the same triage also produces tickets, list them as bullets in the same Adjunct Wave under the same wave heading, tagging each with `(ticket)` after the ID; the dependency-graph code block at the top of the wave should name the tickets too. If items in one wave have **differing activation status** — e.g., held gameplay specs alongside a hold-exempt correctness/proof-integrity ticket — group them under distinct sub-headings within the wave that name the status (e.g., `### Action-Eligible Now (not held)` vs. `### Authored, Awaiting Activation`) rather than a single flat bullet list, and state the differing dispositions in the wave preamble. Use existing "Adjunct Wave: ..." sections in `IMPLEMENTATION-ORDER.md` as the template. If no active `specs/IMPLEMENTATION-ORDER.md` exists (e.g., it was archived), read the most recently archived `archive/specs/IMPLEMENTATION-ORDER*.md` for the structural template; if none exists either, follow the fresh-file structure defined in the `assessment-to-specs` skill (Step 10). **User-directive override**: When the user's invocation explicitly conditions IMPLEMENTATION-ORDER.md creation (e.g., "only if more than one spec"), respect that conditioning. In a single-spec opt-out case, the triage record (when one is being written per the triage-with-substantial-dismissal-record rule) becomes the wave context; the spec stands alone. When the archived template's structure differs from the `### Adjunct Wave`-under-`## Phase` form (e.g., the project retired phase gates in favor of top-level `## Adjunct Wave` headings), the archived template's structure governs — match the project's current convention rather than the literal Phase-placement wording above. Do not scatter triage-derived specs into unrelated waves. **For specs derived from a user feature request (no triage, no report)**: create the appropriate section — a new top-level `## Developer Tooling` for tooling-boundary specs (if one does not already exist), or the relevant Phase for simulation specs — and place the spec bullet under a newly-created `### Adjunct Wave: <topic>` heading. The derivation preamble is replaced by a one-paragraph motivation preamble stating the brainstorm date, the user-stated problem, and the key interview decisions that shaped the spec. For spec deliverables, the "Brainstorm Context" header is replaced by the spec's Problem Statement section, which should include the motivation, evidence sources, and key interview decisions that shaped the spec design.
- If the brainstorm is a **triage producing ≥ 2 deliverables of any combined type (specs + tickets) derived from a named report, scenario run, or remediation wave — OR a triage that dismisses or reaffirms a substantial set of items from a named source report even when it produces only a single new deliverable** (the dismissal/reaffirmation record is itself the durable value, e.g. preventing a future iteration from re-litigating already-decided items), additionally write a short `docs/triage/YYYY-MM-DD-<topic>-triage.md` summarizing: the source report/scenario/finding set, accepted items (with the full path to each written spec or ticket and a one-line rationale), dismissed items (with a one-line reason each), reaffirmed items (items the source re-raises that an existing pending or accepted spec/ticket already covers — name the covering path; no new deliverable results, distinct from both accept and dismiss), and any follow-up work items the triage identified but did not action. Keep the triage file under 150 lines. When the source report or finding set is large (>800 lines) OR the triage produces substantial critical reassessment of the source, the cap rises to ~200 lines. The discipline remains: do not duplicate spec or ticket content; the longer file holds dismissal/reaffirmation reasoning and per-item rationale, not deliverable-internal detail. The triage file makes the brainstorm's decisions durable and shareable without re-running the brainstorm. For a triage producing a single deliverable with no substantial dismissal/reaffirmation record, or for triages whose deliverables are routine work not derived from a named source, skip this file — the individual deliverables and the `IMPLEMENTATION-ORDER.md` update are sufficient history.
...
**If the deliverable is hybrid** (e.g., immediate code + deferred spec, code + tickets, code + multiple specs): compose the relevant per-deliverable substitution rules into a single menu. Place the immediate-implementation option(s) first (e.g., `Apply Deliverable 1 inline now`), followed by deferred-deliverable next-steps from the appropriate per-deliverable template (e.g., for a deferred spec: `Decompose <spec> into tickets` and `Reassess <spec>`). Keep the total to 3–4 options; collapse redundant variants of the same action. Option 4 remains `Done for now`.
...
**If implementation was completed inline**: If the task was simple enough that implementation was completed during or immediately after design approval, skip the menu and summarize what was done. **Spec deliverables do not qualify**: writing a spec is design completion, not implementation completion (the spec activates a future implementation wave). The "implementation completed inline" shortcut applies only to deliverables where the design and the runtime change land in the same edits (e.g., bugfix tickets, doc edits). For spec deliverables, present the spec-deliverable menu from Step 5 even though the spec write IS the immediate output.
