...
| Merchant selling | `merchandise_profile` present | [`trade_actions.rs`](../crates/worldwake-systems/src/trade_actions.rs) | In progress in [§5.9](#59-in-progress-9-survival-trade) |
| Trade negotiation | `trade_disposition` present | [`trade_actions.rs`](../crates/worldwake-systems/src/trade_actions.rs) | In progress in [§5.9](#59-in-progress-9-survival-trade) |
| Commodity valuation | `commodity_valuation` present | [`trade_actions.rs`](../crates/worldwake-systems/src/trade_actions.rs) | In progress in [§5.9](#59-in-progress-9-survival-trade) |
| Substitute preferences | `substitute_preferences` present | [`trade.rs`](../crates/worldwake-core/src/trade.rs) | In progress in [§5.9](#59-in-progress-9-survival-trade); focused AI selection proof landed, but roadmap-row golden isolation remains pending |
...
| Stock / transport | `merchandise_profile` plus stock-supporting world state | stock and transport actions | In progress in [§5.9](#59-in-progress-9-survival-trade) |
...
| In progress in `survival-trade.ron` | Merchant selling, Trade negotiation, Commodity valuation, Stock / transport |
| Structural coverage only, not yet landed | Place concealment, Obligation satiation, Substitute preferences, Disposal, Facility-queue contention, Bounty posting, Notice posting, Theft, Justice / accusation, Violation investigation, Patrol, Pursuit, Combat, Escort, Search |
...
The key constraint is that structural activation alone is not a feature landing. `cli-evaluation.ron`, `survival-tell.ron`, `survival-ask-consult.ron`, and now `survival-trade.ron` can expose future substrate without automatically promoting every structurally active row to `Landed`.
...
| 9 | `survival-trade` | Merchant selling, trade negotiation, commodity valuation, substitute preferences, stock / transport | In Progress | Multi-agent coordination and ownership-sensitive planning |
...
`survival-trade.ron` now owns a truthful roadmap scenario for the trade row. It keeps a full 1440-tick survival-health contract alive while authoring a merchant facility, staged bread stock, buyer purchasing power, `trade_disposition`, `commodity_valuation`, and `substitute_preferences` in one live scenario instead of relying on the old auxiliary merchant-only goldens.

The current golden proves the live seam that actually happens today: `Merchant Sera` commits `stage_stock_for_sale`, a listed bread lot appears at the market, `Buyer Nila` repeatedly commits local `trade` actions against that listed lot, authoritative bread and coin transfer occur at the trade seam, and both agents satisfy the authored survival contract. That is enough to move row 9 out of "planned" and into roadmap-owned progress for merchant selling, negotiated trade, and the stock/display substrate.

This row is not yet landed. Focused AI proof now covers the live substitute-selection seam: candidate generation can emit explicit substitute-backed `AcquireCommodity(SelfConsume)` goals, ranking now prefers the earlier stored substitute order over rival same-category self-consume branches, and exact trade planning still builds an explicit trade payload for the selected commodity. But the roadmap-owned golden remains narrower. `survival-trade.ron` still stages Bread, the buyer's authored `SubstitutePreferences` still list Bread first, and the current golden therefore truthfully proves the bread-market branch rather than a substitute-isolation branch. Commodity valuation and substitute preferences are active supporting substrate, but substitute-driven row landing now depends on a follow-up scenario/golden ticket that isolates the authored substitute branch honestly.
...
### 5.9 In Progress #9: `survival-trade`
...
**Why this scenario is truthful but not yet landed**

The scenario now proves a real survival-era market branch instead of a narrow auxiliary trade harness. `Merchant Sera` stages bread into a listed sale lot at `Market Square`; `Buyer Nila` starts hungry with coin but no direct local food; and the golden proves the concrete causal chain that currently exists in the live runtime: listing becomes visible, repeated `trade` commits transfer Bread and Coin authoritatively, `eat` follows the first successful purchase, and both agents survive the full 1440-tick run.

That proof is deliberately narrower than the full row scope. The planning/runtime substitute seam is now partially landed in focused AI coverage: explicit substitute commodity goals can be generated, same-category selection respects stored substitute order, and exact trade planning keeps the selected branch on an explicit trade payload. But the authored scenario still exposes Bread as the live local market branch and does not isolate a substitute-only path. Until a roadmap-owned scenario/golden proves substitute-driven trade for the authored reason, row 9 stays `In Progress` rather than `Landed`.
