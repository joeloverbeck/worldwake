...
| Substitute preferences | `substitute_preferences` present | [`trade.rs`](../crates/worldwake-core/src/trade.rs) | Planned; structurally active in [§5.9](#59-in-progress-9-survival-trade) but still blocked on explicit planner/runtime integration |
...
This row is not yet landed. `SubstitutePreferences` is structurally active in the scenario, but the runtime still enumerates trade payloads from concrete listed lots in seller `sale_kinds`, and the planner still pursues `AcquireCommodity` for the requested commodity rather than explicitly selecting a valuation-approved substitute candidate. Commodity valuation is therefore active supporting substrate for the proved trade branch, not yet an isolated roadmap landing, and substitute-driven trade remains blocked on a follow-up integration ticket.
...
That proof is deliberately narrower than the full row scope. `SubstitutePreferences` is authored and structurally active, but the current planning/runtime path still binds trade affordances from concrete listed commodities and does not yet turn substitute selection into an explicit new trade proposal. Until that planner/runtime seam is implemented, row 9 stays `In Progress` rather than `Landed`.
