...
| 13 | `survival-justice` | Justice / accusation + violation investigation + report / witness + search | In Progress | Lawful office-holder substrate plus retained accusation and fine seams are now authored; search/report remains blocked behind truthful scenario ownership |
...
- Active follow-up: [`tickets/S13SURJUS-003.md`](../tickets/S13SURJUS-003.md) for search/report stale `ask_about_person` recovery

The scenario is worth retaining because it now owns the row's lawful authority substrate and proves both accusation and fine punishment under the survival loop without helper-only setup. Row 13 still remains `In Progress` because the search/report seam is not yet the truthful retained scenario owner.
