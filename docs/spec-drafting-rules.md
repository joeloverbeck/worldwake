...
6. **Causal-equivalence contract**: Required for any spec introducing offscreen simulation, boundary compression, sleeping entities, region summaries, population approximations, prehistory, or new cache/save-load surfaces. Cite FND-12 and name the explicit higher-fidelity referent, preserved causal variables, admitted error bounds or nondeterminism, materialization/decompression boundary, and comparison tests or audits.
7. **Systemic-validation analysis**: Required for cross-system features. Cite FND-31, declare the negative illegal-path cases the feature must not produce, and name which feature-scoped systemic checks apply, such as invariants, property or metamorphic tests, cross-scenario composition tests, seed or sensitivity sweeps, long-run soaks, replay/save-load equivalence checks, or causal trace audits.

See `specs/FND-01-phase1-foundations-alignment.md` Section H and `docs/FOUNDATIONS.md` Principles 3, 7, 8, 12, 20, and 31 for rationale.
