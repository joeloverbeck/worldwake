...
5. Run the affected crate's tests as the normal broadened proof for the ticket. For Rust tickets, run `cargo fmt --all` before final broad verification; do not pass the active ticket Markdown or other non-Rust files to `rustfmt` unless the ticket explicitly owns a Markdown-formatting step. If the local change adds or changes a dependency, treat the owning manifest and `Cargo.lock` as part of the landed surface: compile the affected target, include both paths in diff/status and closeout, and do not describe the ticket as single-file. If the ticket's Test Plan or repo norms call for CI-matching clippy, run `cargo clippy --workspace --all-targets -- -D warnings` as part of normal broadened verification; use compile/lint fallout to catch remaining shared-shape literals/helpers and local cleanup. That broadened lint surface can also lawfully expose adjacent same-crate `tests/`, bin, or report-helper fallout that the ticket should absorb when it is still part of making the owned verification target pass.
...
## Visibility/derive-only public API enabling tickets

When the ticket only widens an existing type's visibility, adds derives such as `Serialize`/`Deserialize`, or re-exports the type so a later public report/tooling surface can name the real owner type, classify it as `visibility/derive-only public API enabling` instead of a full shared-field migration when all of these are true:

1. The variant set, fields, ordering, semantics, and runtime producers/consumers stay unchanged.
2. The type is not a persisted-shape change beyond the existing type's serialization contract, and no enclosing saveable carrier shape changes.
3. The downstream report, observer, CLI, or schema that will consume the widened type is owned by a later ticket or already exists unchanged.

Use this compact path:

1. Confirm the owner symbol and current visibility/derive set in the live code.
2. Sweep current readers and classify them as in-module, in-crate, external, or absent; verify the change is a widening, not a rename or semantic migration.
3. Check the export surface that downstream code will use, such as a crate-root `pub use`, and avoid exposing sibling private structs/modules unless the ticket explicitly owns that wider API.
4. Add the narrowest focused proof for the trait/serde contract over all variants or representative values. Use an existing dependency such as `bincode` when it proves serde without adding a JSON-only dependency solely for the test.
5. Run the affected crate tests and CI-shaped lint named by the ticket or repo norms.
6. Truth-sync the active ticket and directly cited active spec from "currently private / not serializable" wording to a result-tense before/after record or current-state wording. Keep sibling scans contract-specific: search active sibling tickets for the exact old visibility phrase, old derive claim, dependency id, blocker wording, or code-like snippet, not generic active-ticket headings such as `PENDING`, `What to Change`, or `Test Plan`.
...
7. When a public report, fixture, or schema promises JSON output and stores maps keyed by enums, newtypes, structs, or other non-string-ish keys, verify whether raw JSON object-key serialization is valid for every live key variant. If payload-bearing enum keys or richer key types make that false, rebind the proof to the honest seam: format-agnostic serde for the carrier, plus a renderer- or fixture-owned deterministic JSON representation, and truth-sync downstream observer/fixture tickets that assumed verbatim JSON map keys.
8. Name the honest proof surfaces early: default/serde behavior, authored-input parsing, bootstrap/default seeding, save/load proof when applicable, and the narrowest existing focused tests that already touch the shape.
9. Land the first shared field/type patch, then run `cargo test --workspace --no-run` early to let all-target compile fallout enumerate the remaining exhaustive constructors and helpers.
10. Treat compiler fallout as the source of truth for the remaining shared literal patch set; do not patch default-spread call sites unless the live contract actually requires it.
11. Before running focused Rust tests, resolve exact test IDs with `cargo test -- --list` and prefer module-qualified or `--exact` selectors over loose substrings.
12. Rerun the same broadened command that exposed fallout after each fix; do not rely on only the focused rerun when the broad proof has not yet gone green.
13. Update the active ticket's scope and verification sections to match the real shared-field fallout, especially when reassessment narrows the drafted constructor count or adds focused serde/authored-input/save-load proof.
