...
32. For repo-wide struct-literal or constructor fallout caused by a shared field addition, target exact constructor matches first (for example `EventPayload {` rather than a looser nearby-field pattern), then immediately re-scan touched files for accidental edits in same-shaped blocks before moving on to verification. Treat this cleanup pass as part of the implementation step, not optional polish after tests fail.
