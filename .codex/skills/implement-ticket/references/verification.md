...
For unit tests inside a binary target such as `src/bin/observer.rs`, resolve and run the bin-local target directly: `cargo test -p <crate> --bin observer <filter> -- --list`, then `cargo test -p <crate> --bin observer <filter>`. This avoids a package-level substring that can fan out across unrelated library, binary, and integration-test targets.
