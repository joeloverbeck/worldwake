...
When a drafted proof names `Type::default()` or another convenience constructor and the live shared type does not implement it, decide explicitly during reassessment whether adding that constructor/`Default` impl is part of the owned public or shared API surface. If it is not clearly owned, use an equivalent explicit-construction proof instead and update the ticket's proof text before implementation.
...
When a ticket or spec asks for a new trace "annotation", first classify the live carrier shape before accepting the drafted wording: event-like enum/variant, field on an existing trace struct, enclosing trace read-model, renderer-only output, or derived post-process value. If the requested event-like carrier does not exist on the live branch, truth-sync the ticket/spec to the actual carrier shape before coding instead of inventing a parallel trace path.
...
Before running a multi-root constructor sweep, derive the actual live roots from `rg --files`, crate directories, or another cheap filesystem listing. Do not include non-existent generic roots such as top-level `tests/` unless they exist in the checkout; noisy or nonzero search commands are weaker evidence and can hide real missing-path mistakes.
...
When a ticket adds provenance metadata to an event, trace, or payload carrier, classify the provenance source before widening stored state: `stored on the carrier`, `derivable from an existing canonical runtime object at the emission seam`, or `unavailable without new substrate`. Prefer derivation from the canonical retained object when it preserves the same causal meaning and avoids duplicating state; only widen frame/runtime/core carriers when the provenance cannot be represented from data already in scope at the honest emission boundary.

When a ticket adds typed reference families to an event, trace, or payload carrier, classify each destination family independently before coding: belief refs, record refs, observation refs, entity refs, place refs, action refs, commodity/aspect refs, or any other typed address the new payload can store. A family is representable only when the live emission inputs carry the exact lawful address or it can be derived from an existing canonical object already in scope at that emission seam. If a family is not representable, leave that family empty or narrow the payload contract, and truth-sync the ticket/spec with the representability gap. Never synthesize placeholder entity IDs, record refs, aspects, statuses, source labels, or other plausible-looking typed addresses to satisfy drafted prose.

When a ticket or spec says an emitted payload, trace field, or report value is already computed, first verify whether the live carrier at the emission seam retains that exact value. If the value is only derivable by rerunning an existing deterministic semantic helper, classify that separately from a new computation path: bounded re-evaluation through the existing helper may be truthful, but rewrite `already computed` / `no new computation` claims to the honest boundary such as `no new ranking pass` or `no new belief query`. If the value is not retained or derivable without new inputs, treat the missing carrier as substrate mismatch before coding.

When a drafted focused proof case for an event, trace, or report field is collapsed by the target helper's grouping, deduplication, filtering, or projection boundary, do not force an impossible fixture. Choose proof cases that are representable at the target seam, and record the collapsed case as lower-layer proof, sibling-owned coverage, or explicit out of scope in the ticket/spec closeout.
