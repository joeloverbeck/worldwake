...
- If the ticket's intended focused proof uses `Type::default()` but the live type does not implement `Default`, add a scope item deciding the ownership boundary for that impl. Prefer an explicit literal/factory proof when a new `Default` impl would be broader API surface than the ticket owns; if the impl lands, record it as part of the shared type surface.
- Build the constructor-sweep root list from live paths (`rg --files`, crate directories, or known package roots) before running the search. Avoid adding absent catch-all roots such as top-level `tests/` just because they are conventional in other repositories.
