...
- Use this closeout order when an active ticket will be completed: completed proof commands, semantic ticket/spec truth-sync (`Outcome`, `Deviations`, stale sketches, active spec drift), final `Verification Result`, targeted stale-claim scans and `check_closeout.py`, `Status: COMPLETED`, any post-status prose limited to non-semantic freshness/hygiene notes, Markdown/diff hygiene, then `git status --short`. `Status: COMPLETED` may be changed in the same final ticket-only patch as `Outcome` and `Verification Result` as long as all executable proof has already passed, stale/draft wording has been checked, and no source, generated, scenario, test, or executable edits follow.
