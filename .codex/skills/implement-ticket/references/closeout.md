...
- When completing an active ticket, convert draft-tense sections into result-tense sections if they remain as live headings after implementation. Common patterns: `Verification Layers` -> `Verified Layers`, `Acceptance Criteria` -> `Acceptance Result`, `New/Modified Tests` -> `Added/Modified Tests`, and `Test Plan` -> `Test Plan Result`. The exact headings are less important than removing future-tense or must-pass wording that `check_closeout.py` would flag after the work is complete.
