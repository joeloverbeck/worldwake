...
- Before editing `Status: COMPLETED`, stop and confirm: no source, generated, scenario, test, or executable edits remain; all required executable gates have exited 0; and only ticket/spec prose or hygiene remains.
...
- Before `Status: COMPLETED`, if `Verification Layers` names a proof layer that did not run in the session, either run it, move it to sibling/future ownership, or rewrite it as descriptive/non-required context. Do not leave an unproved layer looking like completed acceptance evidence.
...
- For schema-only / staged substrate tickets, run the compact preflight in `references/ticket-classification.md` before patching: macro/import fallout, generated inventories, default seeding, exact bootstrap deltas, save-version policy, and forwarding/read surfaces are all part of the first scope pass. If the staged substrate adds a required field to `ActionDef` or another shared registry struct, run the shared-field constructor sweep across all `crates/` before patching, not only the crates named in the draft ticket.
  If the staged substrate adds a public value type, method, or enum intended for later sibling-ticket consumption, check crate-root exports and the public namespace surface before closeout instead of relying only on the owner module edit.
  If the staged substrate is an event-log tag or payload meant to be inspectable later, also apply the full event carrier-chain preflight from shared enum/payload routing before accepting a tag-only or standalone-payload draft.
