...
For component-registration and other additive schema tickets, explicitly sweep authoritative inventories and registration proofs before calling the scope final: hardcoded `ALL` arrays, sample value builders, exact create-agent/create-entity delta assertions, and similar exhaustive registration fixtures often remain owned fallout even when the production code change is small.
