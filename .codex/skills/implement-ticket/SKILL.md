...
When a golden ticket's live proof seam changes during reassessment, also update any scenario metadata comments or other doc-feeding annotations inside the golden test file before regenerating inventory/docs. Generated golden docs reflect those metadata blocks, so stale labels there will silently publish the wrong scenario summary even when the executable assertions are correct.
