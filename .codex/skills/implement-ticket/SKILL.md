...
- If a narrow production fix or staged substrate lands but the drafted broader ending is still false, follow `Mixed outcome: narrow fix landed, broader drafted ending still false` below.
- Prefer the strongest existing honest proof seam. Extend an existing focused unit/runtime/golden test instead of creating the drafted new file mechanically; for golden scenario ownership details, load `references/reassessment-golden.md`.
...
- Static declaration, registry, or policy metadata: keep the change on the declaration/registry owner; verify live constant placement, completeness tests, owner-model parity tests, and focused policy/metadata assertions. Do not widen into runtime emitters, ranking, save-shape, or constructor sweeps unless the metadata type shape or live consumers actually change.
...
- After context compaction or resume, re-open this Execution Hard Stops section before any proof-counted command. The next Cargo command must still be standalone and outside `multi_tool_use.parallel`, even if the compacted summary lists several Cargo commands.
...
- Prefer drafting result-tense ticket/spec/sibling truth-sync after the final source diff is stable and before the final broad gate. Keep `Status` non-completed and verification rows provisional until the gate exits; after the gate, flip status and reserve post-gate Markdown edits for clerical fixes or stale-scan findings that do not change the proved source/test/generated surface.
...
- Run Markdown/diff hygiene after final ticket/spec edits, normally `git diff --check`, and use explicit untracked-file whitespace checks for owned untracked files. For `git diff --check --no-index -- /dev/null <path>`, exit code `1` with no output can mean only that the file differs from `/dev/null`; emitted `--check` diagnostics are the whitespace failure signal.
- Normal completion order: draft result-tense closeout while `Status` remains non-completed, run `python3 .codex/skills/implement-ticket/scripts/check_closeout.py <ticket-path>`, fix warnings, apply the status flip as a status-only or status-plus-clerical patch, then rerun `check_closeout.py` after `Status: COMPLETED` so completed-ticket-only warnings are caught before final response. Same-patch completion is allowed only when `check_closeout.py` has already passed for the final closeout prose in the same turn and the status flip does not hide unresolved warnings.
...
- For library unit tests, do not use `--exact` with a bare function name unless a `-- --list` run has shown that exact full id. Prefer `cargo test -p <crate> --lib <module_path>::<test_name> -- --exact` after discovery, or run the bare function-name filter without `--exact` and record the nonzero test set.
- For tests inside `src/bin/*.rs`, use the binary target during selector discovery and exact proof. Prefer `cargo test -p <crate> --bin <bin> -- --list`, copy the full listed id such as `tests::name`, then run `cargo test -p <crate> --bin <bin> tests::name -- --exact`; a package-level bare function name can compile targets while executing zero tests. For a just-authored bin-local test whose module-qualified path is visible in the edited file, a direct exact run is acceptable; if it executes zero tests or the wrong target set, treat it as non-proof and rerun through list-based selector discovery.
...
- For static declaration, registry, or policy metadata tickets, first classify the owner table/constant, any generated or hand-maintained completeness inventory, and the live model/parity tests that must stay in lockstep. Keep sibling-owned runtime emitters, ranking, or golden activation out of scope unless live reassessment proves the metadata change alters those surfaces now.
- For shared enum/payload variant tickets, first classify the owner enum/payload, any sibling event/tag or re-export surface that must stay in lockstep, and the likely exhaustive consumer families (observer/CLI/report/render/test fixtures). Treat the migration as more than an owner-file edit even when the first patch lands in one crate. When a ticket pairs an `EventTag` with a decision/event payload enum, sweep each carrier independently: tag membership may be non-exhaustive while `DecisionEventPayload` or report payload renderers still have exhaustive observer/bin/report matches such as `src/bin/observer.rs`.
  ...
  If live compilation or metadata lookup forces the current ticket to absorb a deliverable drafted for a sibling ticket, stop and record the absorbed subset, the residual sibling owner, the affected proof gates, and the sibling truth-sync requirement before final verification or closeout.
...
- For plan-repair or agent-tick revalidation-seam tickets, use `references/reassessment-planner-ai.md`; name the revalidation input, repair context construction, runtime plan-replacement boundary, event-log proof, and any candidate-composition invariant before broad verification.
...
- For additive fields on persisted `Component` profile types such as `CognitiveProfile` or `PerceptionProfile`, apply the same save-shape discipline as removals unless live code proves the profile is not persisted. `#[serde(default)]` may prove authored/text-serde omitted-field tolerance, but bincode-backed save files still need an explicit current-format version decision.
- For shared trait/read-surface additions, classify the real consumer-facing boundary before editing: facade trait vs owning subtrait, blanket/default implementation, forwarding macro or blanket impl, runtime/per-agent backing type, sibling traits with the same method name, and test doubles that may inherit a default `None`/empty implementation. Prove the accessor through the real runtime/per-agent view when that is the honest consumer path, sweep new method names for existing trait/call-site collisions, and truth-sync ticket/spec wording if the draft names a stale macro, constructor, or implementation layer.
...
When focused live proof confirms a real narrow production fix, staged substrate, or earlier architectural seam inside the ticket's domain but the drafted broader ending still does not hold afterward, load `references/mixed-outcome.md` and follow that playbook. In short: land the narrow owning seam, stop broad scenario/API/architecture churn, rewrite the active ticket/spec to the proved boundary, record the still-false premise, create or update the follow-up owner, clean up temporary probes, and close out with the split explicitly documented. For multi-scenario golden suites where only some scenarios can move to a source-backed seam, use the reference's scenario-by-scenario proof-seam split.
...
4. When the ticket name implies a numbered family or the user cites a parent spec, search for sibling tickets in the same family and confirm whether adjacent missing substrate is already owned elsewhere before broadening or narrowing the current ticket.
   ...
   Before broad multi-root searches, derive or confirm the live roots with `rg --files`, `ls`, or an equivalent cheap listing. Do not include generic roots such as top-level `tests/` unless they exist in the checkout; nonzero search noise can hide real missing-path mistakes.
...
For golden scenario contract, metadata, and generated companion fallout, load `references/verification.md` and `references/reassessment-golden.md`; keep the top-level rule to update scenario metadata, regenerate companions, and escalate to roadmap/golden-policy owners when the authored causal branch itself is underspecified.
...
If a narrow production fix or staged substrate lands and a broader drafted ending is still false afterward, do not keep extending the current ticket in search of a synthetic passing end state; follow `Mixed outcome: narrow fix landed, broader drafted ending still false` above.
...
Hard stop before the status flip: do not set `Status: COMPLETED` until `python3 .codex/skills/implement-ticket/scripts/check_closeout.py <ticket-path>` has passed on the final result-tense closeout prose, unless using the explicit same-patch exception from the Mandatory Closeout Checklist. After flipping to `Status: COMPLETED`, rerun the same checker before final reporting because some warnings only apply to completed tickets.
...
When the ticket lands a truthful narrow fix or staged substrate but a drafted broader premise remains disproved, make that split explicit in closeout by following `Mixed outcome: narrow fix landed, broader drafted ending still false` above.
