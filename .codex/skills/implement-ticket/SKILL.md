...
- Use this closeout order when an active ticket will be completed: completed proof commands, semantic ticket/spec truth-sync (`Outcome`, `Deviations`, stale sketches, active spec drift), final `Verification Result`, `Status: COMPLETED`, any post-status prose limited to non-semantic freshness/hygiene notes, Markdown/diff hygiene, then `git status --short`.
...
5. If the ticket's Test Plan names `./scripts/verify.sh`, inspect the live script during this context-loading pass so its current gates shape implementation constraints, naming choices, and closeout verification.
