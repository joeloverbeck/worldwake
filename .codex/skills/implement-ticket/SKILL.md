...
For broad Rust verification commands that share Cargo build/artifact directories (for example `cargo test -p <crate>`, `cargo test --workspace`, and `cargo clippy --workspace --all-targets -- -D warnings`), prefer running them sequentially rather than in parallel. Parallelize cheap reads/searches and independent focused checks freely, but avoid turning broadened verification into cargo lock contention.
