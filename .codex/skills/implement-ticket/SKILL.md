...
- For S134 or other `ActionDef.effect_schema` tickets, load `references/effect-schema-migrations.md` before source edits. Classify each drafted step as genuinely generic or category-owned during reassessment, and do not flatten domain aftermath into a generic effect step just to satisfy stale spec sketches.
