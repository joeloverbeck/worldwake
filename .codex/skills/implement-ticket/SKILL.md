...
For CLI/tooling-only tickets, if the owned logic can be factored into local helpers, prefer bin-local `#[cfg(test)]` coverage over command-only validation. When the ticket's user-visible contract is report, dump, or formatter output, also add at least one focused assertion against the final render/output surface rather than proving only the helper-level computation.
