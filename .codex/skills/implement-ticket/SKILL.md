...
- For schema-only / staged substrate tickets, run the compact preflight in `references/ticket-classification.md` before patching: macro/import fallout, generated inventories, default seeding, exact bootstrap deltas, save-version policy, and forwarding/read surfaces are all part of the first scope pass.
