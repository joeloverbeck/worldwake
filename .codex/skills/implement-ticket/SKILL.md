...
When a ranking ticket intentionally drives a candidate's motive score to zero, assert the live zero-score carrier instead of expecting a ranked entry with `motive_score == 0`. In `worldwake-ai`, zero-score candidates are filtered out of `RankingOutcome.ranked` and recorded in `RankingOutcome.zero_motive`; focused tests and ticket closeout should name that boundary explicitly.
