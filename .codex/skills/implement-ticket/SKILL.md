...
- Before `Status: COMPLETED`, compare every drafted `Acceptance Criteria`, `Test Plan`, `New/Modified Tests`, and `Verification Layers` bullet against the final proof. Rewrite satisfied bullets to the landed proof, move deferred bullets to a named sibling/future owner, and remove or supersede any unrun command list that could be mistaken for observed verification.
- Add `## Verification Result` if the ticket lacks it. In the final `Verification Result`, label each recorded command as `Passed`, `Waived`, or `Blocked`; when a broad gate ran before later prose-only ticket/spec edits, note the post-edit hygiene or stale-scan proof separately instead of implying the broad gate covered the prose edit.
