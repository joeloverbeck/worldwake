...
- Complete the reviewed ticket's proof-surface blocker scan before marking active specs, implementation-order docs, or roadmap material completed. If archival blocks, only make the active ticket truthful and defer completion-sync edits until the resumed implementation is ready to archive.
...
5. If the just-finished implementation completed multiple tickets in a dependency chain, review each completed ticket as its own archival candidate in dependency order. Archive ready prerequisites when appropriate, repair downstream `Deps` after moves, and block only the tickets whose own implementation or handoff remains incomplete.
6. Confirm the implementation state is present locally (committed or not).
7. Record whether the completed ticket is tracked or untracked in the current worktree so the report can describe archival state accurately.
8. Snapshot `git status --short` and classify dirty paths before review as:
...
9. If the ticket lives under `.claude/worktrees/<name>/`, treat that worktree root as the repository root for all operations.
10. If the target ticket cannot be identified confidently, stop and ask.
...
3. Fix factual, unambiguous handoff issues directly: missing/incomplete `Outcome`, inaccurate verification notes, archival mechanics per [docs/archival-workflow.md](../../../docs/archival-workflow.md).
   ...
   - Before applying active spec, implementation-order, or roadmap completion updates, finish the blocker checks below for ticket prose, source-golden metadata, generated golden docs, executable assertions, ignored-golden ownership, and narrowed proof seams.
4. If unresolved in-scope deliverables remain, stop and report archival as blocked — implementation must resume first.
   ...
   - If correction requires editing `golden_*.rs` comments, test attributes such as `#[ignore = "..."]`, or other source-golden metadata, treat that as a forbidden test/source edit for this review skill. Block archival, make the ticket truthful, and hand the exact file/metadata seam back to `implement-ticket`.
   ...
   - For golden tickets with new, renamed, or newly ignored tests, classify every `#[ignore]` golden as existing-workflow-owned, new/updated-workflow-owned, or manual-only. Verify the source ignore reason, ticket closeout, active spec wording, generated docs, and CI/workflow ownership all agree. If they disagree and the fix is source metadata, block archival and return to the implementation workflow.
...
- **Existing active dependents**: When any pre-existing active ticket's `Deps` field names the just-archived ticket, retarget that dependency to `archive/tickets/<id>.md` as archival mechanics even if the dependent ticket was not created in the current session.
- **Archived prerequisite handoff truthing**: When the reviewed ticket completes or retires a transitional path, placeholder, fallback, or cleanup owner that an archived prerequisite named in current-tense `Outcome` or handoff prose, amend that archived prerequisite narrowly so the older behavior is framed as historical and any cleanup-owner reference points at the now-archived ticket path.
...
- **Stale-scan pattern hygiene**: Keep reference and stale-claim `rg` scans to single-line patterns unless multiline mode is intentional. If you need to check several phrases, split them into alternation on one line or separate `rg` commands so an accidental literal newline does not turn the scan into a non-proof. Avoid look-around syntax in default `rg` patterns because Rust regex does not support it; either use simpler literal/alternation scans or pass `--pcre2` deliberately when look-around is truly needed.
