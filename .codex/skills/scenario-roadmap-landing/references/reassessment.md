...
3. Verify that the row's feature label and activation prose still match the live mechanic contract. If the roadmap wording overstates the shipped seam, narrow the roadmap/generator wording first instead of forcing implementation to satisfy stale editorial text.
   If the wording names an explicit deliverable that live mechanics cannot support as written, do not silently choose a nearby proof target. Apply AGENTS.md's 1-3-1 rule: state the contradiction, offer three viable paths, recommend one, and wait for confirmation before reinterpreting the deliverable.
...
5. Check existing scenarios and goldens to see what is already proven, what is only structurally active, and what is only auxiliary evidence.
   For mechanics that require paired actor/world substrate, such as an agent profile plus a facility/entity policy, verify that both sides are authored and that `scenario_coverage.rs` classifies one-sided presence as inactive or partial rather than active.
