...
const SPLIT_SAVE_FORMAT_VERSION: u32 = 25;
...
pub fn load_from_bytes(bytes: &[u8]) -> Result<(SimulationState, Option<Vec<u8>>), SaveError> {
    ...
    match found {
        ...
        SPLIT_SAVE_FORMAT_VERSION => load_split_v25(payload),
...
fn load_split_v25(bytes: &[u8]) -> Result<(SimulationState, Option<Vec<u8>>), SaveError> {
    let (sim_payload, rest) = split_length_prefixed_payload(bytes, "simulation")?;
    let state = legacy_v25::load_simulation_state(sim_payload)?;
    let (runtime_payload, trailing) = split_length_prefixed_payload(rest, "runtime")?;
    if !trailing.is_empty() {
        return Err(SaveError::Deserialization(
            "save data has trailing bytes after runtime payload".to_string(),
                    ...
                    (
                        *entity,
                        LegacyCombinedProfileV24 {
                            ...
                            snapshot_travel_horizon: cognitive.snapshot_travel_horizon,
                            max_node_expansions: cognitive.max_node_expansions,
...
mod legacy_v25 {
    ...
    struct LegacyCognitiveProfileV25 {
        max_candidates_to_plan: u8,
        max_plan_depth: u8,
        switch_margin: Permille,
        transient_block_ticks: u32,
    ...
    #[derive(Clone, Copy, Debug, Eq, PartialEq, Serialize, Deserialize)]
    struct LegacyExecutionBudgetV25 {
        max_node_expansions: u16,
        beam_width: u8,
        snapshot_travel_horizon: u8,
        max_prerequisite_locations: u8,
    ...
    #[derive(Clone, Debug, Eq, PartialEq, Serialize, Deserialize)]
    struct LegacyComponentTablesV25(
        BTreeMap<EntityId, Name>,
        BTreeMap<EntityId, AgentData>,
        BTreeMap<EntityId, WoundList>,
        BTreeMap<EntityId, CombatProfile>,
        ...
        BTreeMap<EntityId, TellProfile>,
        BTreeMap<EntityId, CommunicationProfile>,
        BTreeMap<EntityId, LegacyCognitiveProfileV25>,
        BTreeMap<EntityId, LegacyExecutionBudgetV25>,
        BTreeMap<EntityId, DriveThresholds>,
        BTreeMap<EntityId, HomeostaticNeeds>,
        BTreeMap<EntityId, DeprivationExposure>,
        BTreeMap<EntityId, MetabolismProfile>,
    ...
    #[derive(Clone, Debug, Eq, PartialEq, Serialize, Deserialize)]
    struct LegacyWorldV25(
        EntityAllocator,
        LegacyComponentTablesV25,
        RelationTables,
        Topology,
    );
    ...
    #[derive(Clone, Debug, Eq, PartialEq, Serialize, Deserialize)]
    struct LegacySimulationStateV25(
        LegacyWorldV25,
        EventLog,
        Scheduler,
        RecipeRegistry,
    ...
    pub(super) fn load_simulation_state(bytes: &[u8]) -> Result<SimulationState, SaveError> {
        let legacy: LegacySimulationStateV25 = bincode::deserialize(bytes)
            .map_err(|error| SaveError::Deserialization(error.to_string()))?;
        legacy.into_current()
    }
    ...
    pub(super) fn save_bytes(
        ...
        bytes.extend_from_slice(&super::SAVE_MAGIC);
        bytes.extend_from_slice(&super::SPLIT_SAVE_FORMAT_VERSION.to_le_bytes());
        bytes.extend_from_slice(&sim_payload_len.to_le_bytes());
        bytes.extend_from_slice(&legacy_payload);
        bytes.extend_from_slice(&runtime_payload_len.to_le_bytes());
        bytes.extend_from_slice(runtime_payload);
    ...
    #[cfg(test)]
    impl CurrentSimulationStateWire {
        fn into_legacy(self) -> LegacySimulationStateV25 {
            let Self(
                world,
                event_log,
                scheduler,
    ...
    #[cfg(test)]
    impl CurrentWorldCompatWire {
        fn into_legacy(self) -> LegacyWorldV25 {
            let Self(allocator, components, relations, topology) = self;
            LegacyWorldV25(allocator, components.into_legacy(), relations, topology)
        }
    }
    ...
    #[cfg(test)]
    impl CurrentComponentTablesWire {
        fn into_legacy(self) -> LegacyComponentTablesV25 {
            let Self(
                names,
                agents,
                wound_lists,
            ...
            let legacy_cognitive_profiles = cognitive_profiles
                .iter()
                .map(|(entity, cognitive)| {
                    (
                        *entity,
                        LegacyCognitiveProfileV25 {
                            max_candidates_to_plan: cognitive.max_candidates_to_plan,
                            max_plan_depth: cognitive.max_plan_depth,
                            switch_margin: cognitive.switch_margin,
                            transient_block_ticks: cognitive.transient_block_ticks,
                ...
                .collect();
            let legacy_execution_budgets = execution_budgets
                .iter()
                .map(|(entity, execution)| {
                    let cognitive = cognitive_profiles
                        .get(entity)
                        .expect("split-format save requires cognitive profile for every execution budget");
                    (
                        *entity,
                        LegacyExecutionBudgetV25 {
                            max_node_expansions: cognitive.max_node_expansions,
                            beam_width: execution.beam_width,
                            snapshot_travel_horizon: cognitive.snapshot_travel_horizon,
                            max_prerequisite_locations: execution.max_prerequisite_locations,
            ...
            LegacyComponentTablesV25(
                ...
                tell_profiles,
                communication_profiles,
                legacy_cognitive_profiles,
                legacy_execution_budgets,
                drive_thresholds,
                homeostatic_needs,
                deprivation_exposures,
                metabolism_profiles,
    ...
    impl LegacyComponentTablesV25 {
        fn into_current(self) -> Result<ComponentTables, SaveError> {
            ...
            let cognitive_profiles = legacy_cognitive_profiles
                .iter()
                .map(|(entity, cognitive)| {
                    let execution = legacy_execution_budgets
                        .get(entity)
                        .expect("split-format migration requires execution budget for every cognitive profile");
                    (
                        *entity,
                        CognitiveProfile {
                            max_candidates_to_plan: cognitive.max_candidates_to_plan,
                ...
                .collect();
            let execution_budgets = legacy_execution_budgets
                .iter()
                .map(|(entity, execution)| {
                    (
                        *entity,
...
mod tests {
    use super::{
        LEGACY_SAVE_FORMAT_VERSION, SAVE_FORMAT_VERSION, SAVE_MAGIC, SaveError, legacy_v24,
        legacy_v25, load, load_from_bytes, save, save_to_bytes,
    };
    use crate::{
    ...
    #[test]
    fn load_migrates_split_v25_format_to_reclassified_profiles() {
        let (mut state, actor, _, _) = populated_state();
        let cognitive_profile = CognitiveProfile {
            ...
            switch_margin: worldwake_core::Permille::new(150).unwrap(),
            transient_block_ticks: 14,
            unknown_block_ticks: 7,
            structural_block_ticks: 240,
            initial_cooldown_ticks: 8,
            max_cooldown_ticks: 96,
        };
        let execution_budget = ExecutionBudget {
            beam_width: 12,
            max_prerequisite_locations: 6,
        };
        set_agent_profiles(&mut state, actor, cognitive_profile, execution_budget);
        let runtime_bytes = [8, 6, 4, 2];

        let (restored, runtime_payload) =
            load_from_bytes(&legacy_v25::save_bytes(&state, Some(&runtime_bytes)).unwrap())
                .unwrap();
