...
mod tests {
        ...
        vec![
            ...
            (
                ...
                DecisionEventPayload::GoalCommitted(GoalCommittedPayload {
                    ...
                    motive_score: 420,
                    decisive_motive_sources: vec![MotiveSourceRef {
    ...
    #[test]
    fn save_format_version_is_92_after_s149_partial_plan_storage_landing() {
        assert_eq!(SAVE_FORMAT_VERSION, 92);
    }
    ...
    #[test]
    fn load_rejects_pre_s149_partial_plan_storage_version_91_without_migration_shim() {
        let (state, _, _, _) = populated_state();
        let mut bytes = save_to_bytes(&state, None).unwrap();
        bytes[SAVE_MAGIC.len()..SAVE_MAGIC.len() + std::mem::size_of::<u32>()]
            .copy_from_slice(&91_u32.to_le_bytes());
