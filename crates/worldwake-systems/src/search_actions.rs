...
use worldwake_core::{
    ...
    EvidenceKind, ExpectationOutcome, ExpectationRecord, ExpectationState, ExpectationStore,
    LastSeenMemory, LastSeenProvenance, LastSeenRecord, SearchCondition, SearchResult,
    VisibilitySpec, World, WorldTxn, is_incapacitated,
};
use worldwake_sim::{
    ...
    CommitOutcome, Constraint, DeterministicRng, DurationExpr, EffectEvaluationContext, EffectMode,
    EffectSink, EffectStep, Interruptibility, PerAgentBeliefView, Precondition, RuntimeBeliefView,
    SearchPlaceActionPayload, TargetSpec, apply_effects_with_context,
};
...
fn search_place_action_def(id: ActionDefId, handler: ActionHandlerId) -> ActionDef {
    ActionDef {
        ...
        effect_schema: worldwake_sim::EffectSchema {
            preconditions: vec![],
            steps: vec![EffectStep::SearchPlace],
        },
    }
}
...
fn commit_search_place(
    ...
    apply_search_place_effect_schema(def, instance, txn)
}

struct SearchPlaceEffectSink<'txn, 'world, 'instance> {
    txn: &'txn mut WorldTxn<'world>,
    instance: &'instance ActionInstance,
    action_error: Option<ActionError>,
}

impl SearchPlaceEffectSink<'_, '_, '_> {
...
impl EffectSink for SearchPlaceEffectSink<'_, '_, '_> {
    ...
    fn search_place(
        ...
        apply_search_place_commit(self.instance, self.txn)
            .map(|_| ())
            .map_err(|err| self.record_error(err))
    }
}

fn apply_search_place_effect_schema(
    def: &ActionDef,
    instance: &ActionInstance,
    txn: &mut WorldTxn<'_>,
) -> Result<CommitOutcome, ActionError> {
    let mut sink = SearchPlaceEffectSink {
        txn,
        instance,
        action_error: None,
    };
