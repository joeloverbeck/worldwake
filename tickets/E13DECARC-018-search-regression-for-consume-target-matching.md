...
3. Existing `search.rs` tests cover successful local consume, adjacent travel-then-consume, and pickup-driven consume paths, but do not currently cover the negative case where only the wrong consumable is locally controllable (confirmed).
4. `AcquireCommodity` search paths are already covered elsewhere. This ticket should stay focused on the regression actually exposed by GOLDENE2E-010 rather than reopening broader consume/acquire semantics.
...
### 2. Add a positive paired regression to prove the correct commodity still works

In the same test module, add or refine a paired assertion that:

- the same fixture with local `Bread` does produce a valid consume plan
...
If the existing positive local-consume test already covers this exact path clearly, it may be referenced instead of duplicated.
...
1. `crates/worldwake-ai/src/search.rs` — add a negative regression for mismatched local consume targets and a positive paired control
