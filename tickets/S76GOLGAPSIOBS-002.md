...
- Build 1 place: FarmVillage with an apple source entity and a water source entity.
- Spawn several ground waste items at FarmVillage to create belief capacity competition.
- Spawn 1 AI agent at FarmVillage with `PerceptionProfile`.
- Run for 50 ticks.
- Collect: whether `known_entities` contains the apple source entity ID and the water source entity ID.
...
1. `golden_perception_forms_resource_source_beliefs` — agent's `known_entities` contains apple source and water source entity IDs after 50 ticks at FarmVillage
