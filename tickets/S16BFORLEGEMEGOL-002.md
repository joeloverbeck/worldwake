...
6. Travel action: A is human-controlled, issued a `RequestAction` for travel via `InputKind::RequestAction`. Travel edges exist in prototype world between VillageSquare and other places.
...
- Pre-seed A's force claim via `txn.add_force_claim(a, office)`.
