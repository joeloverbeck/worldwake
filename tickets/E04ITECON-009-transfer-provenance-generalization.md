...
6. `specs/S04-merchant-selling-market-presence.md` says trade should append traded provenance, but that spec is about preserving trade-specific meaning, not about blessing `Traded` as the generic marker for every transfer. The current codebase can satisfy that intent more robustly by using transfer provenance plus trade-tagged event linkage.
...
4. No backward-compatibility aliases or dual semantics: do not keep both "generic transfer" and "pretend trade transfer" paths alive.
...
### 2. Add a canonical transfer-provenance helper
...
- preserve the current event-linking behavior used by transaction commit

If a more general helper shape is required by the actual transaction/event implementation, keep the public API small and transfer-specific rather than adding another free-form provenance escape hatch.
...
- trade appends `Transferred` provenance and still emits trade-tagged events
- bribe appends `Transferred` provenance and still emits social/transfer-tagged events
...
- redesigning `EventRecord` or the event-log storage model
...
4. Trade still emits `Trade` and `Transfer` event tags.
5. Bribe still emits the existing social/transfer event tags and witness semantics.
6. Core provenance enum tests and serialization tests pass after the rename.
...
3. No dual semantics: after this change, non-trade transfers must not write trade-specific provenance.
4. Conservation and existing transfer behavior remain unchanged.
...
2. [crates/worldwake-systems/src/trade_actions.rs](/home/joeloverbeck/projects/worldwake/crates/worldwake-systems/src/trade_actions.rs) — assert successful trade records `Transferred` provenance while keeping trade event tags.
3. [crates/worldwake-systems/src/office_actions.rs](/home/joeloverbeck/projects/worldwake/crates/worldwake-systems/src/office_actions.rs) — assert bribery records `Transferred` provenance while keeping obligation/perception behavior.
