...
2. Archived tickets 004 and 007 now land the live emission slice for `GoalCommitted`, `PlanAdopted`, `ExpectationMismatch`, `BlockerRecorded`, `GoalOffered`, and `GoalSuppressed`. The remaining S110 variants are deferred to tickets 008-009, so a replay test that claims all-variant coverage still cannot rely on a pure live scenario run yet.
