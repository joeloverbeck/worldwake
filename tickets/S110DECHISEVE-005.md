...
2. Archived tickets 004, 007, and 008 now land the live emission slice for `GoalCommitted`, `PlanAdopted`, `ExpectationMismatch`, `BlockerRecorded`, `GoalOffered`, `GoalSuppressed`, `GoalSuspended`, `GoalAbandoned`, `PlanInvalidated`, and richer `ReplanTriggered`. Ticket 009 lands the first live `RepairApplied` subset (`AlternateTarget`), while ticket 010 owns the richer repair-kind remainder, so full-family replay coverage may still need a synthetic `EventLog` fixture until that final repair-provenance work lands.
