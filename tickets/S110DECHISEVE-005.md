...
2. Archived ticket 004 lands the first live emission slice only: `GoalCommitted`, `PlanAdopted`, `ExpectationMismatch`, and `BlockerRecorded`. The remaining S110 variants are deferred to tickets 007-009, so a replay test that claims all-variant coverage cannot rely on a pure live scenario run yet.
