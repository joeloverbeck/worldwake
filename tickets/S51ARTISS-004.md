...
4. `S51ARTISS-003` is the first ticket that makes autonomous posting candidates live. This ticket therefore depends on `003` for both the CLI showcase weights and the golden proof surface.
...
2. Scenario tuning belongs with the end-to-end closeout once posting behavior is actually live; landing non-zero showcase weights before candidate generation exists would add noise without proving anything.
...
- Candidate emission and ranking logic — ticket 003
