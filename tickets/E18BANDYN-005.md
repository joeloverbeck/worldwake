...
6. The grace period (`abandonment_grace_ticks` on `BanditCampProfile`) prevents premature abandonment when all members are briefly away (e.g., on a raid). This requires tracking "ticks since last member present" — either as a field on `BanditCamp` or as a separate lightweight tracking mechanism.
...
Note: This field addition should be coordinated with E18BANDYN-001. If E18BANDYN-001 is implemented first, this ticket modifies the existing struct.
...
2. Grace period is profile-driven (`BanditCampProfile.abandonment_grace_ticks`), not hardcoded
