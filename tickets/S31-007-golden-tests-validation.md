...
3. `golden_save_load_round_trip_under_ai` already exists in `crates/worldwake-ai/tests/` and remains the live save/load parity surface for the AI runtime including `exhaustion_cache`. The current canonical runtime format is now version 8 after `S31-008`.
...
2. The needs-band production fix now belongs to archived `S31-008`, and the remaining TTL cleanup still belongs in `S31-006`. This ticket should certify the combined end-state only after that remaining retry dependency is resolved.
