...
**Deps**: S116DRIESCSUS-001

## Problem

Spec S116 requires a per-agent profile component `DriveEscalationProfile` carrying `start_after_ticks`, `growth_per_tick`, and `max_multiplier` per `HomeostaticNeedId`. Universal per CLAUDE.md §5 — every agent needs it for motive scoring. Must round-trip through save/load and be constructed by `World::create_agent` so runtime reads via `expect()` succeed. This ticket also adds the pure `escalation_multiplier(ticks, params) -> Permille` helper that the ranking-layer ticket (004) and the needs-system ticket (003) both consume.
...
6. Shared boundary under audit: the `with_component_schema_entries!` macro output — adding one entry here implies generated function names `insert_component_drive_escalation_profile`, `get_component_drive_escalation_profile`, `set_component_drive_escalation_profile`, etc., consumed by ticket 003 (needs_system) and ticket 004 (RankingContext via belief views).

## Architecture Check

1. Universal classification per CLAUDE.md §5: `Default` impl at engine-wide defaults (`start_after_ticks: 100`, `growth_per_tick: Permille(10)`, `max_multiplier: Permille(3000)`), `unwrap_or_default()` in spawn_agent (ticket 005), `.expect()` on known agents in ranking (ticket 004).
...
3. `escalation_multiplier` is a pure function — takes `(ticks, params)`, returns `Permille`. Never persisted; recomputed each ranking pass (FND-27 — derived summaries are caches, never truth).
...
impl Default for DriveEscalationParams {
    fn default() -> Self {
        Self {
            ...
            max_multiplier: Permille::new_unchecked(3000),
        }
    }
}
...
Add `pub use drive_escalation_profile::{DriveEscalationParams, DriveEscalationProfile, escalation_multiplier};` in `crates/worldwake-core/src/lib.rs` and declare `mod drive_escalation_profile;`.
...
1. New bincode round-trip test for `DriveEscalationProfile` (with a non-empty `per_need` map) and `DriveEscalationParams`.
2. New unit tests for `escalation_multiplier`: (a) returns `Permille(1000)` when `ticks <= start_after_ticks`, (b) grows linearly by `growth_per_tick` past the threshold, (c) saturates at `max_multiplier`, (d) handles `u32::MAX` ticks via saturation without panic.
