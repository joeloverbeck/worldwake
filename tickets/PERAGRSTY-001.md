...
4. Per `tickets/README.md` check 13: all macro expansion sites (`delta.rs`, `world.rs`, `component_tables.rs`) must import the new type — the macro generates code using bare type names.
...
2. Component registered for `EntityKind::Agent` -> unit test attaching profile to agent entity and reading it back
...
Verify that all macro expansion sites (`delta.rs`, `world.rs`, `component_tables.rs`) import `ReasoningProfile` — add `use crate::ReasoningProfile;` where needed.
