...
4. Ranking arithmetic the live composite must preserve: `source_trust_weight` is read at `ranking.rs:440, 552` and applied as `effective_discount = trust_weight × failure_ratio / 1000`. The new `wait_sensitivity_weight` (added in ticket 001) is read in the same context. `memory_retention_ticks` (existing field, default 400) bounds the capacity freshness window per spec D4 pseudocode. The composite formula must match the spec: `post = motive × (1000 − trust_discount) / 1000`, then `post = post.saturating_sub(wait_penalty).saturating_add(capacity_signal).max(1)`, with an early `None` return when `post == motive_score` (no observations of any kind).
5. The four existing tests will need behavior updates rather than removal: `source_reliability_discount_skips_non_commodity_goals` is structural (no commodity → no discount) and stays unchanged; the other three currently assert `None` for failure-ratio-zero cases — under the new composite they will return `Some(SourceReliabilityDiscount { ... })` whose `failure_ratio_permille == 0`, `wait_penalty == 0`, `capacity_signal == 0`, and `post_discount_motive == pre_discount_motive`. Per the new early-out (`if post == motive_score return None`), they still return `None`. So the existing assertions remain valid in the spec-compliant case where no observations exist. Extend them to also assert the composite path is exercised when observations *do* exist.
...
8. No heuristic is being removed; the failure-only discount is being subsumed into a strictly more general composite. The early-out removal is a precondition-relaxation: the new computation always runs, but returns `None` when the composite produces no adjustment.
...
2. No backwards-compatibility shim. The early-out is removed; the function's surface (`Option<SourceReliabilityDiscount>` return) is preserved. Old tests that relied on `None` for failure-ratio-zero cases without other observations continue to pass because the new `if post == motive_score return None` covers that case. No deprecated `apply_failure_only_discount` shim is left behind.
...
- Early return: `if post == motive_score { return None; }` — no axis contributed a meaningful adjustment.
...
- Same early `None` return when `post == motive_score`.
...
- `source_reliability_discount_applies_capacity_signal_within_freshness_window`: agent has `last_observed_capacity: 18` at Tick(100), current tick Tick(200), `memory_retention_ticks: 400`; assert `capacity_signal > 0` (75% freshness factor; `18 × 750 / 1000 == 13`) and `post_discount_motive > pre_discount_motive`.
