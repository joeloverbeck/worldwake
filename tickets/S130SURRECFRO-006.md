...
3. `GoalBeliefView::survey_memory(agent)` (added in ticket 004) returns `Option<&SurveyMemory>`; reading the freshest matching record uses `SurveyMemory::find(place, hypothesis)` (added in ticket 002).
