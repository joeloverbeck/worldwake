...
**Deps**: S151TESRELROU-002, S151TESRELROU-003, S151TESRELROU-005

## Problem

S151's D13 bumps `SAVE_FORMAT_VERSION` from 87 to 88 to cover the cumulative serialized-state additions across tickets 002 (runtime store fields on `AgentDecisionRuntime`), 003 (universal components on `EntityKind::Agent`), and 005 (payload context fields on `GoalCommittedPayload` / `GoalSuppressedPayload`). Earlier tickets all use `#[serde(default)]` (or Default-derived `AgentDecisionRuntime`) to keep pre-bump save streams deserializable; this ticket coordinates the version bump as the single visible boundary.
...
3. The cumulative new serialized state covers:
   - Ticket 002: `AgentDecisionRuntime.testimony_reliability: TestimonyReliability` and `AgentDecisionRuntime.route_preference: RoutePreference` (both Default-derived, so old saves load into empty stores).
   ...
   - Ticket 005: `GoalCommittedPayload.testimony_trust_context: Vec<_>`, `GoalCommittedPayload.route_preference_context: Vec<_>`, `GoalSuppressedPayload.testimony_trust_context: Vec<_>` (all `#[serde(default)]`, so old saves load into empty vecs).
4. All upstream tickets use Default-derived or `#[serde(default)]` so that landing them individually does not break load on a pre-87 save — the bump is the official "save format now includes S151 state" boundary, not a hard incompatibility break.
5. **No load-migration code required**: because every new field deserializes cleanly from the absence of its bytes (Default semantics), old save bytes load into empty/default values for the new fields without explicit migration. The bump is a forward marker so subsequent format changes can rely on S151 state being present.
...
3. Per FND-12 (performance compresses computation, never causality): old saves still load into a legal game state because the new fields all have safe defaults — no causal divergence from re-loading a pre-S151 save into a post-S151 binary.
...
3. Backward-compat → load a pre-bump (87) save and assert all new S151 fields are present with default/empty values, without errors.
...
Confirm `load_current_format` at `save_load.rs:129` handles the new fields via existing serde machinery — no per-field branching expected because the new fields all use Default semantics.
...
Add a sibling test loading a pre-bump byte fixture (constructed in-test as a version-87 minimal save) and asserting it loads with empty S151 fields:
...
If constructing a version-87 fixture in-test is impractical, document the omission and rely on the production deserialization tolerance verified by tickets 002, 003, and 005.
...
- Any production-code migration logic (none required; all new state uses Default semantics)
...
3. Backward-compat test: pre-bump save loads into default S151 state (or documented omission with rationale).
...
3. Saves written by version-87 binaries load by version-88 binaries with default S151 state.
...
Merge note: This ticket bumps SAVE_FORMAT_VERSION 87→88 as a single-coordinated step after tickets 002, 003, and 005 land their `#[serde(default)]`-protected serialized state. No sibling tickets bump the version; deliberate avoidance via `#[serde(default)]` on every new field.
