...
**Deps**: S144AGGSCEDIA-004, S144AGGSCEDIA-005
...
1. Adding Section 13 alongside the existing 12 sections and the anomaly channel keeps the observer a pure read-only consumer — it calls `build_scenario_diagnostics` and renders, never mutating world state. The JSON format emits the report verbatim, reusing its serde derives.
...
2. JSON format emits the report such that it parses back to an identical `ScenarioDiagnosticsReport` -> focused unit test (serialize via renderer → deserialize → equal).
...
Add `render_scenario_diagnostics_section(report: &ScenarioDiagnosticsReport, format: DiagnosticsFormat, out: &mut impl Write) -> io::Result<()>` following the existing `render_*_section` pattern. Text format renders tables with top-N capping + "...others" summary; JSON format emits the report verbatim via serde.
...
2. JSON format output parses back to an identical `ScenarioDiagnosticsReport`.
...
2. JSON output is the report verbatim — round-tripping it yields an equal `ScenarioDiagnosticsReport`.
