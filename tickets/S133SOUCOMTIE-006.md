...
2. Spec D6 dictates the six new scenario contracts. Each scenario's planner surface: `GoalKind::AcquireCommodity { commodity: CommodityKind::Apple, purpose: CommodityPurpose::SelfConsume, .. }` (or `CommodityKind::Water` for thirst-driven variants). The orchard scenario fixtures already exist in the `golden_harness` test infrastructure shared by the S131 goldens.
3. Shared abstraction boundary under audit: end-to-end behavior — the agent's chosen source under planned conditions, plus the `RankedGoalComparisonDimension` attribution surfaced through the decision trace.
4. Failing-golden invariants: the four pre-existing goldens encode "drink", "force-law uptake", "diversification alive under survival", and "wash relief lands by row five" — all of which depend on cross-category goal ranking (Wash/Drink vs AcquireCommodity) being unaffected by source-of-acquisition perturbation.
...
12. Scenario isolation choice: the new file's six scenarios deliberately exclude political/social/combat affordances that could lawfully compete for the agent's tick budget. Scenarios use minimal worlds (one or two places, two same-commodity sources, no theft/duty/office substrate) so the only contended dimension is the source choice.
...
1. Same-commodity wait reranking → golden E2E (agent picks far orchard); decision trace attributes `RankedGoalComparisonDimension::SourceComposite`.
2. Cross-category neutrality → golden E2E (Wash beats AcquireCommodity when motive higher); decision trace attributes `MotiveScore`, never reaches `SourceComposite`.
...
6. No-record neutrality → golden E2E with a fresh agent; both sources resolve to `composite_permille = 1000`; comparator falls through to `PlaceKey`/`EntityKey` for deterministic ordering.
...
1. `same_commodity_wait_reranking_picks_far_orchard_when_close_orchard_has_observed_waits`
   ...
   - Expect: agent commits AcquireCommodity at FAR orchard; trace attributes `RankedGoalComparisonDimension::SourceComposite`; close orchard's `wait_factor_permille` < 1000.
...
2. `cross_category_neutrality_keeps_wash_above_acquire_apple_when_motive_higher`
   ...
   - Expect: agent commits Wash; trace attributes `MotiveScore`; the comparator never reaches `SourceComposite`.
...
3. `fresh_capacity_bonus_picks_higher_capacity_orchard_when_motive_tied`
   ...
   - Expect: agent commits AcquireCommodity at the 18-capacity orchard; trace attributes `SourceComposite`; high-capacity factor > 1000, low-capacity factor between 500-1000.
...
5. `empty_but_fresh_observation_demotes_depleted_source`
   - Two orchards at equal cost; one's most recent observation is `last_observed_capacity = 0` and within freshness window.
...
6. `no_record_neutrality_falls_through_to_lower_tiebreakers`
   ...
   - Expect: both sources resolve to `composite_permille = 1000`; comparator falls through to `PlaceKey`/`EntityKey` for deterministic ordering. Trace attributes a non-`SourceComposite` decisive dimension on the final compare.
...
- Engine changes — covered by tickets 001–005.
