...
4. Existing inline tests that exercise the changed surfaces and must be updated: `plan_repair.rs::downgrade_to_progress_barrier_preserves_committed_prefix_only` (line 705), `plan_repair.rs::discrepancy_clearing_dispatch_covers_all_variants` (484), `plan_repair.rs::repair_kind_attempt_order_is_deterministic` (420); `scenario_diagnostics/mod.rs` test fixture at line 229 (`PlanTerminalKind::GoalSatisfied`); the `search/tests.rs` assertions (24 `ProgressBarrier` references).
5. Live planner surface: typed terminals are search outcomes produced by `terminal_kind` in `search/transition.rs`; the search layer knows its termination reason at each site, so each `ProgressBarrier` construction maps to the typed terminal matching its cause (budget-out → `SearchBudgetExhausted`). Richer barrier-fact derivation (commodity/place/authority specifics) is deferred to ticket 004 — this ticket maps each site to the closest typed terminal the search layer can determine locally.
...
2. No backward-compat alias: `ProgressBarrier` is removed outright, not deprecated. `DowngradeToProgressBarrier` is renamed in place (single live representation), not shimmed. The discriminant is a derived projection (`From<&PlanTerminalKind>`), not a parallel authoritative type — it is a histogram key only (FND-27).
...
3. Repair downgrade path intact after rename → existing `plan_repair.rs::downgrade_to_progress_barrier_preserves_committed_prefix_only` (renamed) passes against `RepairKind::DowngradeToTypedBarrier`.
...
### 3. Migrate `ProgressBarrier` removal + `DowngradeToProgressBarrier` rename (D3)

- Replace each `ProgressBarrier` construction in `search/transition.rs` and `search/mod.rs` with the typed terminal matching its local cause (budget-out → `SearchBudgetExhausted`).
...
1. New: `terminal_kind` returns `SearchBudgetExhausted { .. }` when search exhausts budget, and the other typed variants for their respective causes.
2. New: two `PlanTerminalKind` values differing only in payload map to the same `PlanTerminalKindDiscriminant` and aggregate to one `terminal_kind_distribution` bucket.
