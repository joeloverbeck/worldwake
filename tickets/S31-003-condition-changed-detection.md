...
3. `FacilitiesChanged` and `BlockerExpired` use conservative `true` defaults per spec Section 6: they always invalidate when present in conditions. Refinement via dirty-bit delegation is deferred to implementation.
...
2. Conservative defaults for `FacilitiesChanged`/`BlockerExpired` are safe (may over-invalidate slightly) and can be refined without API changes.
...
7. Conservative fallback for FacilitiesChanged/BlockerExpired -> unit test
...
- `FacilitiesChanged`: `true` (conservative)
- `BlockerExpired`: `true` (conservative)
...
- Refined dirty-bit delegation for `FacilitiesChanged`/`BlockerExpired` (future optimization)
...
12. Unit test: `FacilitiesChanged` always returns `true` (conservative)
13. Unit test: `BlockerExpired` always returns `true` (conservative)
...
3. Conservative conditions (`FacilitiesChanged`, `BlockerExpired`) always return `true` — safe default

## Test Plan

### New/Modified Tests

1. `crates/worldwake-ai/src/exhaustion.rs` (inline `#[cfg(test)]`) — 13 unit tests covering every condition variant with mock `GoalBeliefView`
