...
4. This is not an AI regression ticket. Candidate generation, affordance discovery, and planner op mapping remain in `E16BFORLEGJURCON-007/008`; this ticket only changes the authoritative office-control system that those layers eventually rely on.
...
10. Golden isolation is not in scope here. `E16BFORLEGJURCON-009` already notes that goldens for these grace fields must wait until production semantics exist.
...
2. No backward-compatibility aliasing or parallel force paths. The same canonical force-control system should absorb the grace semantics rather than adding wrappers or special-case branches elsewhere.

## Verification Layers

1. Vacancy claim grace delays controller establishment -> authoritative world state (`office_controller` remains `None`, `OfficeForceState` reflects pending state) + focused system test
2. Challenger presence grace delays contested transition -> authoritative world state (`office_controller` remains current controller during grace) + focused system test
3. Challenger persistence through grace clears controller and marks contested -> authoritative world state + focused system test
4. Challenger departure before grace expiry preserves controller continuity -> authoritative world state + focused system test
5. Political trace reflects pending-grace states distinctly from active control/contest -> focused politics trace assertions
6. Installation gate still uses `uncontested_hold_ticks` after grace windows complete -> authoritative world state + focused system test
...
- AI affordance enumeration or planner ops — `E16BFORLEGJURCON-007/008`
- Golden E2E coverage for the new grace semantics — `E16BFORLEGJURCON-009`
...
1. `cargo test -p worldwake-systems force_control_`
