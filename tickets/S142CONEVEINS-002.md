...
**Priority**: HIGH
**Effort**: Medium
**Engine Changes**: Yes — `worldwake-core` typed-blocker enum payload widening; `worldwake-ai` cross-crate migration of all 17 use sites
**Deps**: archive/tickets/S142CONEVEINS-001.md (provides `AffordanceKey`)

## Problem

S142's headline causal chain is "agent's `ReservationConflict` blocker → resolved by which contention event?". Today the chain is broken: `BlockingFact::ReservationConflict` is a unit variant at `crates/worldwake-core/src/blocker_memory.rs:197` carrying no affordance identity and no reference to the resolving event. Without `affordance: AffordanceKey`, the AI cannot disambiguate between two simultaneous reservation conflicts on different facilities. Without `contention_event: Option<EventId>`, decision traces cannot point a debugger to the resolution that produced the loss. This ticket widens the variant from unit-form to struct-form and migrates the 17 cross-crate construction and destructuring sites in `worldwake-ai`.

## Assumption Reassessment (2026-05-10)

<!-- Apply all domain-specific precision rules from docs/precision-rules.md -->

1. `BlockingFact::ReservationConflict` is a unit variant at `blocker_memory.rs:197` (verified). Workspace-wide use sites total 17, all in `worldwake-ai`: 1 in `agent_tick/mod.rs:235` (exhaustive match arm), 16 in `failure_handling.rs` (split by the `#[cfg(test)]` boundary at `:1488` into 10 runtime sites — `:420`, `:525`, `:541`, `:736`, `:771`, `:838`, `:959`, `:1234`, `:1250`, `:1450` — and 6 test sites — `:2704`, `:2912`, `:2944`, `:3277`, `:3311`, `:3593`). Zero sites in `worldwake-sim`, `worldwake-systems`, `worldwake-cli`, or `worldwake-core` outside the variant definition.
...
4. `BlockingFact` derives `Copy, Clone, Debug, Eq, Ord, PartialEq, PartialOrd, Hash, Serialize, Deserialize` (verified pattern matches sibling typed enums in `worldwake-core`). `AffordanceKey` is `Copy` (per ticket 001's derive set). `Option<EventId>` is `Copy` (`EventId` is a wrapper around `u64`). Therefore `BlockingFact` retains `Copy` after this widening.
...
1. Variant shape change — focused unit test in `failure_handling.rs` constructs the new struct variant and asserts field roundtrip.
...
3. Existing 7 inline tests in `failure_handling.rs` updated to construct/match the struct form — focused runtime coverage already in place; the test names enumerated in Assumption Reassessment item 2 are the targets.
...
1. The 7 named inline tests in `failure_handling.rs` (per Assumption Reassessment item 2) pass after migration to struct form.
2. New focused test: a `BlockingFact::ReservationConflict { affordance, contention_event }` value roundtrips through `serde` serialization with both fields preserved.
...
4. Existing suite: `cargo test -p worldwake-ai`, `cargo test -p worldwake-core`, `cargo test -p worldwake-sim`.

### Invariants

1. `BlockingFact` retains `Copy` (`AffordanceKey` is `Copy`, `Option<EventId>` is `Copy`).
2. No `BlockingFact::ReservationConflict` site outside `crates/worldwake-ai/` and `crates/worldwake-core/src/blocker_memory.rs` exists after this ticket — verified by post-implementation `grep -rn "BlockingFact::ReservationConflict"`.
...
1. `crates/worldwake-ai/src/failure_handling.rs` (existing `#[cfg(test)]` block) — update 7 named test functions per Assumption Reassessment item 2; add 1 new focused test for the struct-form roundtrip.
2. `crates/worldwake-sim/src/save_load.rs` (existing `#[cfg(test)]` block) — add current-format struct-form round-trip test.
