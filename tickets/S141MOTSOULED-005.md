...
4. Save-format: this ticket does NOT bump `SAVE_FORMAT_VERSION` further. It rides under the 77→78 bump from 002 via `#[serde(default)]` on the new field (defaulting to `Vec::new()`), so pre-bump saves that lack the field still load correctly. The bump is single-shot per the S141 reassessment's merge note in 002.
...
3. `#[serde(default)]` on the new field is the FND-28-compliant boundary shim — it lives at the deserialization boundary only and does not introduce a backward-compat path in the live authority code. The bump in 002 signals forward-incompatibility; the serde default handles backward-compat loading.
...
4. Backward-compatible deserialization → a version-77 byte stream with the pre-S141 `GoalCommittedPayload` shape loads under version 78 with `decisive_motive_sources` populated from `#[serde(default)]` (empty `Vec`).
...
- `SAVE_FORMAT_VERSION` bump — owned by 002 (single-shot bump 77→78 covering all S141 serialized-state changes via `#[serde(default)]`).
...
3. Cross-version deserialization: a version-77 byte stream of pre-S141 `GoalCommittedPayload` loads under version 78 with `decisive_motive_sources` defaulted to an empty vec via `#[serde(default)]`.
...
3. No `SAVE_FORMAT_VERSION` bump in this ticket — confirms the single-shot bump invariant from 002.

## Test Plan

### New/Modified Tests

1. `crates/worldwake-core/src/decision_event_payload.rs#[cfg(test)]` — extend the existing payload round-trip test to cover the new field; add a cross-version-deserialization test for version 77 → 78.
