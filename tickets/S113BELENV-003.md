...
4. **Decision payload population** (`agent_tick/mod.rs` invalidation emission plus blocker-memory persistence sites) — `S113BELENV-002` landed the optional `belief_snapshot` field and save-format bump, but the live runtime emitters still lawfully write `None`. Once this ticket starts emitting `BeliefStale` / `BeliefContradicted` from revalidation and probe-related paths, those same paths should capture the live envelope into the payload so decision history stays reconstructible.
...
5. Populating `belief_snapshot` on the already-extended payload types is cleaner than leaving the decision-history contract staged indefinitely once these exact branches become envelope-aware. The payload shape is already live via `S113BELENV-002`; this ticket owns wiring the first real producers rather than creating a second follow-up for the same branch family.
...
4. Belief-driven `BlockerRecordedPayload` / `PlanInvalidatedPayload` emitted from the newly wired branches carry `belief_snapshot: Some(...)` with the live envelope metadata → focused runtime/decision-payload tests on the affected `agent_tick` emission seams.
5. No action-trace layer is required for this ticket — the integrations are reasoning-layer plus decision-history enrichment. Scenario-level proof is deferred to T005 (golden).
...
### 4. Decision-event payload population on the newly wired branches

On the revalidation / blocker-emission paths that now branch on the envelope:

- When `PlanInvalidatedPayload` is emitted for a belief-driven invalidation owned by this ticket, populate `belief_snapshot: Some(BeliefSnapshot { confidence, status, acquired_tick })` from the same envelope read that triggered the branch.
- When a blocker-memory persistence site emits `BlockerRecordedPayload` for `Discrepancy::BeliefStale` or `Discrepancy::BeliefContradicted` on the newly wired paths, populate the same snapshot field instead of leaving `None`.
- Reuse the existing `BeliefSnapshot` / `BeliefStatusTag` schema from `S113BELENV-002`; no payload-shape changes are needed here.
...
- **agent_tick emission tests** — the affected invalidation/blocker payloads carry `belief_snapshot: Some(...)` with the expected `confidence`, `status`, and `acquired_tick`.
...
- `crates/worldwake-ai/src/agent_tick/mod.rs` and/or `crates/worldwake-ai/src/agent_tick/execution.rs` (modify — populate `belief_snapshot` on the newly belief-driven invalidation/blocker emission paths)
...
1. Focused unit tests for ranking, revalidation, feasibility probe, and the newly populated decision-payload seams per §5 pass.
...
3. Newly belief-driven `BlockerRecordedPayload` / `PlanInvalidatedPayload` emissions from this ticket's branches populate `belief_snapshot` instead of leaving `None`.
4. Probe returns `Plausible` for every envelope state other than `Stale` (and `Contradicted` if the implementer chose the belt-and-braces variant).
...
4. `crates/worldwake-ai/src/agent_tick/tests.rs` — focused payload assertions proving populated `belief_snapshot` on the newly wired branches.
