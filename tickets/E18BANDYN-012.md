...
10. Mismatch + correction: the repo already has good lower-layer tests for `establish_camp`, so the right follow-up is not “add more golden assertions until the omission becomes obvious.” The right follow-up is to add structured bandit omission diagnostics at the same layer where political and social families already expose them.
...
### 2. Surface those omissions through decision traces

Extend `decision_trace.rs` goal-status reporting so `GoalKind::RegroupWithFaction` and `GoalKind::EstablishBanditCamp` can report concrete omission reasons the same way political/social goals already do.
...
Add focused candidate-generation regressions that assert the omission reasons directly, and add the narrowest golden assertion needed in T22 to prove the improved debugging contract without bloating the scenario.
...
4. `golden_t22_bandit_camp_destruction` proves the trace can explain at least one intentionally omitted bandit branch without source-diving.
...
1. `crates/worldwake-ai/src/candidate_generation.rs` — add focused omission-reason regressions for `RegroupWithFaction` and `EstablishBanditCamp`.
2. `crates/worldwake-ai/src/decision_trace.rs` — add focused goal-status coverage for bandit omission reasons.
3. `crates/worldwake-ai/tests/golden_t22_bandit_camp_destruction.rs` — add one narrow assertion proving the improved mixed-layer debugging contract.

### Commands

1. `cargo test -p worldwake-ai candidate_generation::tests::establish_bandit_camp_requires_local_controlled_edible_supplies -- --nocapture`
2. `cargo test -p worldwake-ai golden_t22_bandit_camp_destruction -- --nocapture`
3. `cargo test -p worldwake-systems bandit_camp_actions::tests::establish_camp_reuse_expands_same_place_camp_capacity_for_new_supplies -- --nocapture`
