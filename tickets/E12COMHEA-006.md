...
Authoritative checks for wound load, incapacitation, and death-imminence are derived from `WoundList` + `CombatProfile`. These derived computations need shared helper functions so that combat, scheduling, action validation, and AI all use the same logic. Per Principle 3, these are read-model functions, never stored state.
...
4. `DeadAt` will exist as a component — per E12COMHEA-002.
