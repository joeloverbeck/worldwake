...
After all sub-module extractions, `agent_tick/mod.rs` still contains 59 inline tests (starting at line 2092) and `search/mod.rs` contains 65 inline tests (starting at line 955). These should be moved to dedicated `tests.rs` sub-modules for each directory module, keeping test code near but separate from production code.

## Assumption Reassessment (2026-03-22)

1. `agent_tick.rs` has `#[cfg(test)] mod tests { ... }` starting at line 2092 with 59 `#[test]` functions — verified via `grep -c '#\[test\]'`.
2. `search.rs` has `#[cfg(test)] mod tests { ... }` starting at line 955 with 65 `#[test]` functions — verified via `grep -c '#\[test\]'`.
...
Move the entire `#[cfg(test)] mod tests { ... }` block (lines 2092–EOF) from `agent_tick/mod.rs` into `agent_tick/tests.rs`. In `mod.rs`, add:
```rust
#[cfg(test)]
mod tests;
```
...
Move the entire `#[cfg(test)] mod tests { ... }` block (lines 955–EOF) from `search/mod.rs` into `search/tests.rs`. In `mod.rs`, add:
```rust
#[cfg(test)]
mod tests;
```
