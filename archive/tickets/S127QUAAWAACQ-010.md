...
**Priority**: MEDIUM
**Effort**: Small
**Engine Changes**: `worldwake-sim` (belief-view extension), `worldwake-ai` (clearing baseline + place-scoping guard)
...
1. **Confirmed gap**: removing `transient_block_ticks: Some(2)` from
   Goldens 1 and 5 reproduces the described stall — the slot frees but
   queued actors never re-emit, leaving `granted: None` with the queued
   actors stuck in `waiting`. Auto-applied during implementation.
2. **Diagnosed deeper cause**: `derive_clearing_condition` was not even
   reaching the `ReservationConflict` arm, because
   `record_failure_classification` calls
   `should_scope_local_commodity_unavailability_to_place` which sets
   `target = None` for harvest start failures at single-source places
   (the source is excluded from the place's commodity-support check, so
   the place "lacks" commodity by exclusion). With `target = None`,
   `derive_clearing_condition` returns `TtlOnly`, so position-based
   clearing was never possible. The ticket's assumption that the issue
   was purely the `facility_queue_position` legacy reading was
   incomplete; the fix needed to additionally guard the place-scoping
   logic against contention failures, since contention failures (the
   source exists and has commodity, just busy) are not place-availability
   failures. Auto-corrected: `Engine Changes` updated to include the
   place-scoping guard; `What to Change` and `Out of Scope` clarified.
3. **No `WorkstationBusy` / `ExclusiveFacilityUnavailable` regression**:
   these blocker classes use the legacy `ContentionQueue` substrate, not
   `ResourceExtractionQueues`. The new accessor returns `None`/`false`
   for non-extraction facilities, so legacy flows still hit
   `facility_queue_position` / `facility_grant` / `reservation_ranges`
   unchanged.
...
- Added `extraction_slot_queue_position`, `actor_holds_extraction_slot_grant`,
  `extraction_slot_available`, and `has_extraction_queues` accessors on
  `TemporalBeliefView` (defaulted to inert) with concrete impls on
  `PerAgentBeliefView` reading `ResourceExtractionQueues`.
- `derive_clearing_condition` now reads the contention position from
  whichever substrate is present (extraction queues preferred, legacy
  contention queue as fallback) for all three contention-class blockers.
- `is_blocker_cleared` for `ReservationConflict` delegates to a new
  `reservation_conflict_cleared` helper. On extraction sources it
  clears when the actor was promoted to a slot grant, when any slot
  becomes available, or when their queue position drops; on legacy
  workstations it falls back to the original `reservation_ranges`
  emptiness check.
- Guarded `should_scope_local_commodity_unavailability_to_place` so it
  no longer nulls the target for `ReservationConflict`,
  `ExclusiveFacilityUnavailable`, or `WorkstationBusy`. Without this
  guard, harvest start failures at single-source places lost their
  facility identity and forced TTL-only expiry.
- Dropped `transient_block_ticks: Some(2)` overrides on
  `golden_single_slot_queue_forms_with_concrete_wait` and
  `golden_scenario_e_queue_abandonment_promotes_next_actor`. Updated the
  helper doc and removed stale rationale comments.
- Added 4 focused unit tests covering position-decrease, grant
  promotion, queue-abandonment + slot-available, and the negative
  no-change case; refreshed the existing
  `derive_clearing_condition_contention_blockers_capture_queue_baseline_when_available`
  expectation by adding a parallel
  `derive_clearing_condition_reservation_conflict_uses_extraction_slot_position`
  test for the harvest-source path.
- Refreshed the
  `crates/worldwake-cli/tests/fixtures/observer_decision_history/survival_baseline_5_ticks.md`
  golden: tick-1 BlockerRecorded now carries the well's
  `target: Some(EntityId)` instead of `None`, and tick-4 Agent B no
  longer shows the `GoalSuppressed(AcquireCommodity{Water})` line
  because the blocker has cleared via the new substrate signal.

## Deviations

- Scope absorbed the place-scoping guard
  (`is_contention_blocker` + early-skip in
  `record_failure_classification`). The original ticket did not name
  this fix, but it was necessary for the new accessors to be reached at
  all on harvest start failures at single-source places. The fix is
  small, local, and unambiguously correct (contention is not commodity
  unavailability).

## Verification Result

- Passed `cargo test -p worldwake-ai failure_handling::tests`
  (40 tests including 5 new `is_blocker_cleared_*` /
  `derive_clearing_condition_*` cases).
- Passed `cargo test -p worldwake-ai` (full crate suite).
- Passed `cargo test -p worldwake-cli --test observer_decision_history`
  after refreshing the survival-baseline 5-tick fixture.
- Passed `./scripts/verify.sh` (`cargo fmt --check`,
