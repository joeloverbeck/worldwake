...
2. Spec source: `archive/specs/S150-cross-goal-blocker-scoping.md` D8 (`blocker_counts_by_scope: BTreeMap<BlockerScopeVariantId, u64>` with `BlockerScopeVariantId` defined alongside the existing `CandidateSuppressionCategory` precedent at `scenario_diagnostics/mod.rs:90-108`).
