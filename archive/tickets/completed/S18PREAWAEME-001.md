...
8. `crates/worldwake-ai/tests/golden_supply_chain.rs` still covers only the harvest-based restock segment for apples; the craft-based restock segment named in `archive/specs/S18-prerequisite-aware-emergent-chain-goldens.md` remains unimplemented and is correctly left to later tickets.
