...
4. The ordering contract here is mixed-layer: same-snapshot harvest selection on the same local workstation, followed by authoritative action-lifecycle divergence at start time, then next-tick AI reconciliation. In the current production architecture, ordinary harvest actions reserve the workstation at start and only consume `ResourceSource.available_quantity` at commit in [crates/worldwake-systems/src/production_actions.rs](/home/joeloverbeck/projects/worldwake/crates/worldwake-systems/src/production_actions.rs). That means the losing same-tick contender is expected to fail first through the shared start gate (`ReservationUnavailable` / recoverable start failure) rather than immediate source depletion. The later remote fallback branch should emerge after reconciliation and after the winner's committed local harvest makes the local opportunity non-competitive or depleted, not because the loser gets a special production-only recovery path.
...
6. Current docs already describe the intended gap in [specs/S15-start-failure-emergence-golden-suites.md](/home/joeloverbeck/projects/worldwake/specs/S15-start-failure-emergence-golden-suites.md). The active ticket set now also includes sibling S15 follow-on tickets `S15STAFAIEME-002` through `S15STAFAIEME-006` in `tickets/`; none contradict this production-scenario scope, but the older "only template and README" assumption is stale and corrected here.
...
3. The current architecture is already the cleaner one here: a shared start-gate failure plus ordinary blocker handling is more extensible than inventing a production-specific "losing contention" code path. The golden should prove that architecture, not replace it.
...
4. The winner's committed local harvest lawfully changes the scarce local opportunity after the start failure -> authoritative workstation/source state and, when needed, action-trace ordering rather than inferred tick gaps.
5. Recovery proceeds through distant travel, harvest, and hunger relief -> authoritative world state and action trace commits for travel/harvest/eat.
6. Commodity totals remain bounded by explicit source stock -> authoritative world-state conservation checks, not delayed event-log inference.
...
Use a finite local orchard that can satisfy exactly one eventual committed harvest and a distant fallback orchard reachable through the normal topology. Assert:
...
- the local opportunity is later consumed or rendered non-competitive through the winner's lawful harvest outcome, not a test-side mutation
...
3. `crates/worldwake-ai/tests/golden_care.rs` — minimal existing-test lint cleanup required so the crate-scoped `cargo clippy -p worldwake-ai --tests -- -D warnings` gate passes.
...
4. `cargo test -p worldwake-ai golden_resource_exhaustion_race -- --exact`
5. `cargo test -p worldwake-ai golden_care_pre_start_wound_disappearance_records_blocker -- --exact`
6. `cargo clippy -p worldwake-ai --tests -- -D warnings`

## Outcome

- **Completion date**: 2026-03-19
- **What actually changed**: Added `golden_contested_harvest_start_failure_recovers_via_remote_fallback` and its deterministic replay companion in [crates/worldwake-ai/tests/golden_production.rs](/home/joeloverbeck/projects/worldwake/crates/worldwake-ai/tests/golden_production.rs). The new golden proves both agents initially select the same local harvest branch, one loses at the shared start gate with `StartFailed`, the next AI tick consumes `planning.action_start_failures` and records a reservation-conflict blocker, and the losing agent later satisfies hunger via the distant orchard. Also made a minimal existing-test lint cleanup in [crates/worldwake-ai/tests/golden_care.rs](/home/joeloverbeck/projects/worldwake/crates/worldwake-ai/tests/golden_care.rs) so the requested crate-scoped lint gate passes.
- **Deviations from original plan**: No production or harness changes were needed. The most important reassessment result held: in the current harvest architecture the contested same-tick loser fails first through shared reservation/start-gate handling, while local source depletion happens later at the winner's harvest commit. The landed golden therefore proves the real layered contract instead of forcing a production-specific failure path.
- **Verification results**: `cargo test -p worldwake-ai golden_contested_harvest_start_failure_recovers_via_remote_fallback -- --exact`, `cargo test -p worldwake-ai golden_contested_harvest_start_failure_recovers_via_remote_fallback_replays_deterministically -- --exact`, `cargo test -p worldwake-ai golden_resource_exhaustion_race -- --exact`, `cargo test -p worldwake-ai golden_care_pre_start_wound_disappearance_records_blocker -- --exact`, `cargo test -p worldwake-ai --test golden_production`, and `cargo clippy -p worldwake-ai --tests -- -D warnings` all passed.
