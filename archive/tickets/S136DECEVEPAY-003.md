...
Spec D6 (rejection_dimension slice): `RejectedAlternativeSummary.rejection_dimension` (added by ticket 001) was populated as `None` at the runtime construction site. The decisive ranking dimension is available through the planner's existing comparator explanation seam — `RankedGoalComparisonOutcome::decisive_dimension` exists at `crates/worldwake-ai/src/ranking.rs:2367` and is surfaced by `explain_ranked_goal_order`. Wiring it through into the payload makes the dimension that ordered each rejected goal against the chosen plan part of always-on history, satisfying the success-path side of the FND-29A reconstruction goal without a new ranking pass or belief query.

## Assumption Reassessment (2026-05-06)

1. `build_rejected_alternatives` lives at `crates/worldwake-ai/src/agent_tick/planning.rs:931-1002`. It receives `ranked_candidates: &OrderedRanked<'_>`, `portfolio`, `chosen_goal_key`, `committed_motive`, and `max_alternatives: u8`, and produces `Vec<worldwake_core::RejectedAlternativeSummary>`. Before this ticket, the runtime site initialized `rejection_dimension: None`; after this ticket, summaries emitted through this helper carry `Some(...)` whenever the committed ranked entry and an unequal pairwise comparison are available.
2. `RankedGoalComparisonOutcome` at `ranking.rs:2367` carries `decisive_dimension: RankedGoalComparisonDimension`. `OrderedRanked` stores the ordered ranked entries but does not retain pairwise comparison outcomes against the chosen plan. The live crate-visible seam is `ranking::explain_ranked_goal_order(left, right)`, which wraps the internal `ranked_goal_ordering` helper and returns the decisive dimension for an unequal pair. This ticket owns calling that existing explanation helper per emitted rejected goal; this is not a new ranking pass or belief query.
...
4. Existing test `emit_plan_selection_events_records_commit_then_adoption_with_truncation:3464` exercises `build_rejected_alternatives` indirectly and now asserts `MotiveScore` dimensions on emitted alternatives. Other synthetic `RejectedAlternativeSummary` fixtures outside the helper may keep `rejection_dimension: None` when they do not have a real ranked comparison available.
...
1. `rejection_dimension` is a derived projection of the existing ranking comparator — no new ranking pass, no new belief query, no new authoritative state (FND-3, FND-27).
...
Update the helper at `planning.rs:931-1002` so each rejected candidate produces its dimension. `OrderedRanked` does not carry pairwise outcomes against the chosen plan, so use the existing crate-visible `ranking::explain_ranked_goal_order(&rejected_entry, &committed_entry)` seam per emitted rejected goal and map its `decisive_dimension`.
...
fn ranked_goal_comparison_dimension_tag(
    d: crate::ranking::RankedGoalComparisonDimension,
) -> worldwake_core::RankedGoalComparisonDimensionTag {
    use crate::ranking::RankedGoalComparisonDimension as Src;
...
```rust
.map(|rejected| {
    let dim = crate::ranking::explain_ranked_goal_order(rejected.entry, committed_entry)
...
- `crates/worldwake-ai/src/agent_tick/mod.rs` (modify — non-semantic clippy annotation on the S136 `emit_replan_triggered` helper exposed by `./scripts/verify.sh`)
- `crates/worldwake-ai/src/ranking.rs` (verify only — no changes; confirmed the callable live seam is `explain_ranked_goal_order`, not the private `ranked_goal_ordering` helper)
...
2. New focused unit test on `build_rejected_alternatives` covering at least 2 dimension cases that are representable after the helper's `GoalKey` deduplication (motive-score loss and feasibility loss) — names the live `GoalKind` under test. Same-commodity source-composite alternatives collapse to one `GoalKey` before this helper emits summaries, so `SourceComposite` remains a ranking-level proof surface rather than this payload helper's focused case.
...
- `build_rejected_alternatives` now preserves the rejected `AgendaEntry` used to build each emitted summary and maps its pairwise comparison against the committed ranked entry through `ranking::explain_ranked_goal_order`.
- Added the AI-side `RankedGoalComparisonDimension -> RankedGoalComparisonDimensionTag` conversion at the emission site, keeping the `core <- ai` dependency direction intact.
- Extended `emit_plan_selection_events_records_commit_then_adoption_with_truncation` and added `build_rejected_alternatives_records_decisive_dimensions` to prove emitted `MotiveScore` and `Feasibility` dimensions.
- Added a narrow `#[allow(clippy::too_many_arguments)]` on `agent_tick::emit_replan_triggered`; this helper already had the S136 assumptions payload shape, and the annotation was required for the ticket's CI-matching wrapper gate.

## Deviations

- `OrderedRanked` does not retain per-rejection comparison outcomes, so the implementation uses the existing `ranking::explain_ranked_goal_order` helper per emitted rejected alternative instead of reading a stored outcome.
- The drafted `SourceComposite` focused case is not representable in `build_rejected_alternatives` because the helper deduplicates alternatives by `GoalKey`; the focused helper proof covers `MotiveScore` and `Feasibility`, while `SourceComposite` remains covered at the ranking comparator layer.

## Verification Result

- Passed `cargo test -p worldwake-ai --lib agent_tick::planning::tests::build_rejected_alternatives_records_decisive_dimensions -- --exact`
...
- Passed `./scripts/verify.sh` after the clippy annotation; the live wrapper ran `cargo fmt --all -- --check`, `cargo test --workspace`, `bash scripts/check_active_goal_removed.sh`, `cargo clippy --workspace`, `cargo clippy --workspace --all-targets -- -D warnings`, and `cargo run -p worldwake-cli --bin scenario-coverage -- --check`.
