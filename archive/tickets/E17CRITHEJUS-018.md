...
1. why one ranked goal beat another inside `compare_ranked_goals()`, especially across political/social boundaries
...
6. The live `GoalKind` family under test is mixed: `GoalKind::ShareBelief { .. }` versus political goals including `GoalKind::SupportCandidateForOffice { .. }` and `GoalKind::ClaimOffice { .. }`. Reassessment against `crates/worldwake-ai/src/ranking.rs::priority_class()` shows a crucial correction: political goals are currently `GoalPriorityClass::Medium`, while `GoalKind::ShareBelief { .. }` is `GoalPriorityClass::Low`. So the live architecture does not support a narrative where a political goal loses to social relay chatter on an otherwise equal branch; when those goals are compared directly, the first decisive boundary is priority class.
7. Ordering matters here, but the contract is decision-layer ranking ordering and action-lifecycle ordering, not strict tick separation. The compared branches are not symmetric under the live architecture because political goals and `ShareBelief` already diverge at priority class before feasibility, motive score, and topic tiebreakers are considered. The ticket must therefore expose and verify the actual decisive comparison reason, not assume a nonexistent same-class motive duel.
...
14. Mismatch + correction:
    - the problem is not “action traces lack Tell delta data.” The lower-level data already exists; the ticket scope is to expose and verify that data on the high-signal debugging surfaces actually used in mixed-layer investigations.
    - the planning trace does not currently have a `top_challenger` field parallel to `InterruptTrace`; the live write path is `ranked_candidates` in `crates/worldwake-ai/src/agent_tick/mod.rs` -> `CandidateTrace.ranked` plus `SelectionTrace.selected`. This ticket therefore needs an explicit top-ranked-vs-runner-up comparison surface for planning, rather than assuming an already-existing planning `top_challenger` slot.
    - the original “political goal loses to `ShareBelief`” framing is stale under the current architecture. Live ranking gives political goals higher priority class than `ShareBelief`, so the robust scope is to surface the decisive comparison reason generically and verify it against the live ordering, including political-over-social priority-class cases.
    - any implementation that enriches planning or interrupt traces must touch `crates/worldwake-ai/src/agent_tick/mod.rs`, because that module constructs `DecisionOutcome::{Planning, ActiveAction}` from live ranked candidates.

## Architecture Check

1. The clean architecture is to make the social-decision path self-explanatory at the same level as the world-state path. If information-carrying artifacts and ranking choices drive emergence, the trace model should expose one reusable structured comparison/accessor surface derived from the existing ranking and commit-trace truth, rather than duplicating ranking logic in ad-hoc test helpers or scenario-specific summaries.
...
1. Ranked-goal comparison explanation for political/social and same-class ranking pairs at the planning/interrupt trace boundary -> focused `decision_trace.rs`, `ranking.rs`, and `agent_tick` trace tests
2. Tell commit mutation class (`EntityBelief` / `InstitutionalBelief` / `SocialObservation` / `Mixed` / `None`) remains visible through a typed action-trace query surface and the one-line summary -> focused `action_trace.rs` tests
...
- Extend `crates/worldwake-ai/src/decision_trace.rs` and `crates/worldwake-ai/src/agent_tick/mod.rs` so:
  - planning traces can expose why the highest-ranked goal beat the immediate ranked runner-up
  - active-action interrupt traces can expose why the top challenger currently outranks the interrupted branch
...
- Update `crates/worldwake-sim/src/action_trace.rs` summary helpers and add related typed-query helpers accordingly.
...
- Add focused ranking/decision-trace tests proving the decisive comparison reason is queryable in structured form for live political/social ordering, not merely inferable from the selected slot.
...
- `crates/worldwake-ai/tests/golden_emergent.rs` (modify only if the strongest targeted trace assertion lands there instead of `golden_offices.rs`)
- `crates/worldwake-ai/tests/golden_social.rs` (modify only if workspace lint verification requires a minimal unrelated test cleanup)
...
1. Decision traces can explain why one ranked goal beat another using structured comparison data at the planning/interrupt trace boundary, including the live political-over-social priority-class case
2. Action traces can expose Tell `belief_delta` and `result` through a typed high-signal summary/query surface without hiding the authoritative `TellCommitTrace`
...
1. `crates/worldwake-ai/src/decision_trace.rs` — add focused structured assertions for final ranked-goal comparison rationale. Rationale: prove summaries surface the decisive comparison boundary rather than only the selected result.
2. `crates/worldwake-ai/src/ranking.rs` — add focused comparison-helper assertions for both same-class motive ordering and live political-over-social priority-class ordering. Rationale: prove the explanation helper matches the ranking substrate it reports on.
3. `crates/worldwake-ai/src/agent_tick/tests.rs` — add a runtime trace test that exercises a real planning tick with both political and social candidates and asserts the structured comparison data at the live `agent_tick` construction boundary. Rationale: prove the comparison survives runtime trace assembly against the current architecture rather than an obsolete scenario narrative.
4. `crates/worldwake-sim/src/action_trace.rs` — add focused tests for Tell summary/query access to `belief_delta` and `result`. Rationale: prove Tell mutation-class visibility without unpacking nested commit payloads manually.
5. `crates/worldwake-ai/tests/golden_offices.rs` — strengthen one political golden to assert on the improved trace surface before falling back to world-state checks. Rationale: make the trace layer a first-class debugging contract for office/political regressions.

### Commands

1. `cargo test -p worldwake-ai decision_trace::tests:: -- --nocapture`
2. `cargo test -p worldwake-sim action_trace::tests:: -- --nocapture`
3. `cargo test -p worldwake-ai agent_tick::tests::trace_planning_records_political_over_share_belief_priority_class_reason -- --exact`
4. `cargo test -p worldwake-ai ranking::tests::support_candidate_uses_social_weight_times_loyalty -- --exact`
5. `cargo test -p worldwake-ai ranking::tests::explain_ranked_goal_order_reports_priority_class_for_political_over_social -- --exact`
6. `cargo test -p worldwake-ai --test golden_offices golden_bribe_support_coalition -- --exact`
7. `cargo test -p worldwake-ai --test golden_offices golden_force_control_locality_requires_tell -- --exact`
8. `cargo test -p worldwake-ai --test golden_emergent golden_already_told_recent_subject_does_not_crowd_out_untold_office_fact -- --exact`

## Outcome

Completed: 2026-03-26

- Added reusable structured ranking-comparison reporting in `worldwake-ai` and threaded it into planning and interrupt decision traces without changing ranking semantics.
- Added typed Tell commit accessors in `worldwake-sim` so tests and debugging helpers can query `result` and `belief_delta` directly from action-trace events.
- Strengthened focused and golden coverage around ranking explanations and Tell mutation-class traceability.
- Corrected the ticket scope during implementation: live ranking gives political goals higher priority class than `ShareBelief`, so the completed work exposes the real decisive comparison reason instead of preserving the stale “political loses to social chatter” narrative.
- Minor deviation from original plan: `crates/worldwake-ai/tests/golden_social.rs` received a one-line unrelated lint cleanup so `cargo clippy --workspace --all-targets -- -D warnings` would pass.

Verification results:

- `cargo test -p worldwake-ai decision_trace::tests:: -- --nocapture`
- `cargo test -p worldwake-sim action_trace::tests:: -- --nocapture`
- `cargo test -p worldwake-ai agent_tick::tests::trace_planning_records_political_over_share_belief_priority_class_reason -- --exact`
- `cargo test -p worldwake-ai ranking::tests::support_candidate_uses_social_weight_times_loyalty -- --exact`
- `cargo test -p worldwake-ai ranking::tests::explain_ranked_goal_order_reports_priority_class_for_political_over_social -- --exact`
...
- `cargo test -p worldwake-ai --test golden_emergent golden_already_told_recent_subject_does_not_crowd_out_untold_office_fact -- --exact`
