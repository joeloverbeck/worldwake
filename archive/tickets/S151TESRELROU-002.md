...
4. Spec D3+D4 at `archive/specs/S151-testimony-reliability-and-route-preferences.md:124-179` defines the store types, entry shapes, and derived `trust()` / `preference()` views. All field types resolve to `worldwake-core` symbols (TopicScope from ticket 001, EntityId/Tick/EventId/Permille/RouteSegment all core).
