...
**Deps**: archive/tickets/S151TESRELROU-001.md, archive/tickets/S151TESRELROU-005.md, archive/tickets/S151TESRELROU-006.md
...
4. **Aggregator data flow**: the pre-ticket aggregator incremented `source_reliability_changes` at `aggregator.rs:314` from a source-expectation outcome. The landed per-topic breakdown reads `TestimonyTrustSummary.topic` from `GoalCommittedPayload.testimony_trust_context` and `GoalSuppressedPayload.testimony_trust_context`, then increments the matching `TopicScope` bucket. `route_preference_changes` increments from `GoalCommittedPayload.route_preference_context` entries.
...
2. Flat-field removal → grep workspace for `source_reliability_changes` after the edit; only the `_by_topic` form remains in code.
3. Fixture round-trip → `golden_scenario_diagnostics_harness` passes with the regenerated `expected-scenario-diagnostics.json`.
4. Observer rendering → unit test asserts the topic map renders in the diagnostics report.
...
The test fixture at `mod.rs:231` uses the landed field names.
...
- Added an increment site for `route_preference_changes` when a `GoalCommittedPayload.route_preference_context` is non-empty.
- Line 447: update the `BeliefMetrics` construction to assign both new fields.

The final decision-event read path uses `GoalCommitted` and `GoalSuppressed` testimony trust context, because those payloads carry the `TopicScope` required by the new diagnostics contract.
...
The renderer's accompanying test fixture matches the field shape.

### 4. Regenerate `expected-scenario-diagnostics.json`

Regenerated the diagnostics harness fixture with `WORLDWAKE_UPDATE_SCENARIO_DIAGNOSTICS_FIXTURE=1`; the JSON includes both fields with the correct empty/zero baseline values.
...
5. Observer renderer produced a topic-by-topic list for the map field.
...
- Passed: `crates/worldwake-ai/src/scenario_diagnostics/aggregator.rs#[cfg(test)]` adds focused coverage for per-topic incrementing and route-preference incrementing.
- Passed: `crates/worldwake-ai/src/scenario_diagnostics/mod.rs#[cfg(test)]` updates the existing test fixture.
- Passed: `crates/worldwake-ai/tests/golden_scenario_diagnostics_harness/mod.rs` passes against the regenerated fixture.

### Commands Passed

- Passed: `cargo test -p worldwake-ai scenario_diagnostics`
- Passed: `cargo test -p worldwake-ai golden_scenario_diagnostics`
...
Implemented the S151 D10 diagnostics extension. `BeliefMetrics` now replaces the flat `source_reliability_changes: u64` field with deterministic `source_reliability_changes_by_topic: BTreeMap<TopicScope, u64>` and adds `route_preference_changes: u64`. The diagnostics aggregator rolls up `GoalCommittedPayload.testimony_trust_context` and `GoalSuppressedPayload.testimony_trust_context` into per-topic reliability-change buckets, and counts committed route-preference context entries from `GoalCommittedPayload.route_preference_context`. The observer renderer now emits the topic map plus the route-preference counter, and the scenario diagnostics fixture was regenerated with the new JSON shape.

Deviation from the draft: the final per-topic reliability counter is sourced from the S151 testimony trust contexts on committed and suppressed goal events. It no longer counts the older `SourceExpectationFailure` attribution outcome, because that payload does not carry the `TopicScope` needed for the new diagnostics contract.

Verification:

- `cargo fmt --all`
- `cargo test -p worldwake-ai scenario_diagnostics`
- `WORLDWAKE_UPDATE_SCENARIO_DIAGNOSTICS_FIXTURE=1 cargo test -p worldwake-ai golden_scenario_diagnostics_survival_baseline_fixture_is_stable -- --ignored`
- `cargo test -p worldwake-ai golden_scenario_diagnostics_survival_baseline_fixture_is_stable -- --ignored`
- `cargo test -p worldwake-cli scenario_diagnostics`
- `cargo test -p worldwake-ai golden_scenario_diagnostics`
- `cargo test --workspace`
- `cargo clippy --workspace --all-targets -- -D warnings`
- `cargo fmt --all -- --check`
- `rg -n "\bsource_reliability_changes\b" crates/worldwake-ai crates/worldwake-cli crates/worldwake-core` (no matches)
- `git diff --check`
- `./scripts/verify.sh`
