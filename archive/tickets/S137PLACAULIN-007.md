...
2. Spec `archive/specs/S137-plan-causal-links-and-repair.md` D6 specifies the routing: invoke repair first; on `Repaired`, emit `EventTag::RepairApplied` with the chosen kind and replace the plan; on `Failed`, record attempts in `RepairMemory` and fall through to `handle_current_step_failure`.
