...
**Priority**: HIGH
**Effort**: Medium
**Engine Changes**: Yes — `worldwake-ai` acquisition candidate generation direct resource-source feasibility.
**Deps**: Follow-up to S158 (belief-view remote truth-leak closure). Introducing change: `bedbce48` (`archive/tickets/S158BELVIEWLEAK-001.md`), which gated `listed_sale_lots_at` / `seller_for_sale_lot` / `has_sale_listing` on local visibility in `crates/worldwake-sim/src/per_agent_belief_view.rs`.
...
3. Shared boundary under audit: the acquisition planner surface that turns an `AcquireCommodity(commodity, SelfConsume)` goal into anchored candidate branches (trade-at-market vs. harvest-at-resource). The landed contract is enforced in `crates/worldwake-ai/src/candidate_generation.rs`: a direct resource-source acquisition branch whose terminal action requires a registered input-free harvest recipe is viable only when the actor knows a matching recipe and satisfies its workstation/tool requirements. No `search.rs`, `ranking.rs`, or agenda-pending change was needed.
...
### 1. Recipe-feasibility gate on direct resource-harvest acquisition branches

In acquisition candidate generation (`crates/worldwake-ai/src/candidate_generation.rs`), direct `ResourceSource` evidence now routes through `resource_source_supports_acquisition()` and `known_harvest_recipe_supports_source()`. For direct acquisition search, a registered no-input harvest recipe for the commodity gates the resource-source branch unless the actor knows that recipe and can satisfy its source/workstation/tool requirements. Recursive recipe-input evidence keeps its existing production-path semantics.

### 2. Local-wait / pending-purchase preference investigation result

With the infeasible orchard branch removed, the existing candidate/planning/agenda behavior satisfies the survival-trade contract. No `RevivalTrigger::CounterpartyAvailable` or pending-purchase code change was required in this ticket.

## Landed Files

- `crates/worldwake-ai/src/candidate_generation.rs` — added the direct resource-source feasibility gate and a focused candidate-generation regression. Nearby quantity-aware resource-source tests now seed the known harvest recipe/tool requirement they exercise.
- `crates/worldwake-ai/tests/scenarios/survival_trade.rs` — checked as the motivating golden surface; no test edit was required.
- `crates/worldwake-ai/src/search.rs` / `crates/worldwake-ai/src/ranking.rs` — no change; candidate generation was the earliest owning boundary.
...
3. Focused candidate-generation coverage: an agent lacking the required harvest recipe does not receive a viable harvest-at-resource branch for `AcquireCommodity`, while a co-located trade branch remains preferred.
...
- Added `candidate_generation::tests::remote_harvest_source_without_known_recipe_does_not_emit_acquire_branch`.
- Re-ran the focused survival-trade ignored goldens and the full release ignored `golden_ai` lane. Both survival-trade cases and the full 61-test ignored lane passed.

## Outcome

Completed on 2026-05-21.

- Direct resource-source acquisition evidence now checks registered harvest-recipe feasibility before emitting a direct `AcquireCommodity(SelfConsume)` branch.
- The local seller-backed Apple acquisition branch remains viable when the actor lacks the Apple harvest recipe, so Buyer Nila no longer commits to the recipe-infeasible South Orchard branch.
- The existing S158 remote sale-listing leak closure stayed intact; the fix did not reopen remote listing visibility.

## Deviations

- The drafted search/ranking and pending-agenda investigation did not require code changes. Live proof showed candidate generation was the earliest owning boundary.
- The implementation preserves existing recipe-input evidence behavior for production-path searches; the new recipe-feasibility gate applies to direct resource-source acquisition branches.

## Verification Result

- Passed `cargo test -p worldwake-ai --lib candidate_generation::tests::remote_harvest_source_without_known_recipe_does_not_emit_acquire_branch -- --exact`
- Passed `cargo test -p worldwake-ai --lib candidate_generation::tests::remote_direct_harvest_source_emits_acquire_without_duplicate_produce_goal -- --exact`
- Passed `cargo test --release -p worldwake-ai --test golden_ai -- --ignored --test-threads=1 scenarios::survival_trade::survival_trade_proves_substitute_market_branch`
- Passed `cargo test --release -p worldwake-ai --test golden_ai -- --ignored --test-threads=1 scenarios::survival_trade::survival_trade_replays_deterministically`
