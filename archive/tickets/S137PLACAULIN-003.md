...
**Deps**: archive/specs/S137-plan-causal-links-and-repair.md (D8)
...
2. Spec `archive/specs/S137-plan-causal-links-and-repair.md` D8 enumerates all 20 call sites with the variant mapping table and the `substitute_recipe` field addition. `SAVE_FORMAT_VERSION` was `80` after S137PLACAULIN-002; this ticket bumped it to `81` because the variant rename changes serialized representation (no `#[serde(rename)]` shim, per FND-28).
