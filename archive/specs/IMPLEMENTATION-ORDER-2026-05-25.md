...
- What actually changed: the first AI-architecture improvements wave was fully
  implemented and each accepted spec (`S165`, `S166`, `S167`, `S168`) was moved
  to `archive/specs/` with completed status.
- Deviations from original plan: S166 was narrowed from the source report's
  broader canonical-substrate proposal to the implemented source-fidelity seam;
  S168 remained the lowest-benefit optimization and landed after the higher
  benefit wave items.
- Verification results: this order file records the completed and archived spec
  paths. Implementation verification remains recorded on the individual archived
  specs and tickets.
