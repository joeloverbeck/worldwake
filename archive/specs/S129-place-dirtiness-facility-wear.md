...
2. **`generate_candidates`** (`crates/worldwake-ai/src/candidate_generation.rs`) — `emit_wash_goal` is rewritten per D10 to emit per-basin candidates with `OpportunityAnchor::Entity(basin_id)`. Each candidate must reflect the basin's current `clean_water_units` and `dirtiness_level` so ranking can score them.
...
5. **`handle_plan_failure`** (`agent_tick.rs`) — when the new precondition rejects, the existing start-failure path records the failure and triggers replanning. Selecting a different basin depends on the later per-basin candidate split in D10; D7 owns the graceful failure handoff, not basin-specific recovery by itself.
...
- `emit_wash_goal` (currently at lines 3313–3390) — split per-basin: enumerate all `WorkstationTag::WashBasin` facilities reachable through `wash_access_opportunities()`, emit one candidate per basin with `OpportunityAnchor::Entity(basin_id)`. Each candidate evidence carries the basin's place id for travel cost reasoning. Replaces the current single-place-anchored candidate.
