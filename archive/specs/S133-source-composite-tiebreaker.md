...
| FND-27 (Derived Summaries Are Caches, Never Truth) | The composite is explicitly a derived score, not stored truth. Once D4 carries it on `AgendaEntry`, it is serialized only as runtime save-cache state under `AgentDecisionRuntime.agenda_state`; the authoritative truth remains the stored `ReliabilityRecord`. The fix S133 represents is precisely a re-classification of S131's D4 from "motive truth" back to "derived per-tick view." |
...
4. **Stored state vs. derived read-model.** Stored: the existing `ReliabilityRecord` fields (no change). Derived: `SourceCompositeRank { trust_factor_permille, wait_factor_permille, capacity_factor_permille, composite_permille, source_entity, commodity }`, computed per-candidate per-tick by D2 and surfaced through D5. It is not authoritative world state and is not persisted to the event log. Once D4 carries it on `AgendaEntry`, it is included in the runtime save payload as part of `AgentDecisionRuntime.agenda_state`; D4 therefore owns the current-format save version bump even though the underlying truth remains the stored reliability record.
