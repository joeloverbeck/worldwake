...
- **S115**: Agenda Manager with Goal Lifecycle — `AgendaState` with `committed`/`pending`/`suspended` entries, each with origin, freshness, revival trigger, kill condition; rename `GroundedGoal` → `GoalOffer`, `RankedGoal` → `AgendaEntry`; live ticket 005 lands `tick_agenda` at the post-ranking caller seam plus classifier-driven parking into the real agenda maps. The stronger `agenda_tick_system before candidate generation` plan was falsified on the live branch. Executable commitment still finalizes at selected-plan adoption, and downstream planning still consumes the fresh ranked feed. S115 also owns the *feasibility-rejection lifecycle classifier* (D4A) that decodes S112's `RejectedBeforeSearch { reason }` into `Satisfied` / `InfeasibleUntil { trigger }` / `Dead` lifecycle states, replacing the S112-era `search_order` special case that kept rejected committed opportunities in the search queue.
  - soft dep on S123: `tick_agenda` still consumes fresh ranked `AgendaEntry` values at the caller seam; the merged agenda-manager ordered pool is not yet the live downstream planning feed.
