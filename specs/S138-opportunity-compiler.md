...
3. `AgentDecisionTrace` carries the per-agent-tick `compiled_opportunities: Vec<Opportunity>` read-model alongside the load counter. This is derived diagnostic trace data, not persisted simulation state, and exists so observer/report consumers can render the actual opportunities rather than only aggregate counts.
