...
The simulation remediation report (`reports/simulation-remediation.md`) identified three golden test gaps and one missing component field. This spec adds: (1) a multi-agent convergence test verifying agents at barren locations with remote resource beliefs don't collapse into prolonged sleep+relieve loops, (2) an agent death traceability test verifying death from unmet needs is explicit, causally traceable, and halts post-death planning, and (3) a resource-source affordance test verifying the harvest-to-consume chain works when resource sources exist at an agent's location. As a prerequisite deliverable, this spec adds a `DeathCause` field to the `DeadAt` component so death events carry traceable cause information.
...
- S79 (resource-source consumption affordances) — GT-3 requires the harvest-to-consume chain to work; without S79, GT-3 will fail by design
...
- GT-3 verifies the root-cause fix from S79: harvest affordances at resource source locations
