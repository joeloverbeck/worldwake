...
A returned `ResumedPlan` identifies the suspended agenda entry and updated `PartialPlanSegment` that is eligible for retry. S149PARPLASEG-010 landed the executable planner re-entry contract by re-admitting the segment's stored `GoalOffer` to the existing belief-backed agenda/ranking/planning path after resume conditions hold. It does not replay `remaining_skeleton` directly, because the live skeleton carrier still lacks resolved `ActionDefId`s, authoritative targets, payloads, and planner context.
...
When a plan terminal is `CoordinationBarrier { contested_resource }`, the agenda manager derives a per-actor "watching" list from suspended `PartialPlanSegment`s keyed on `contested_resource`; it does not add a second persisted agenda field. Resume is triggered by the existing contention lifecycle: `ContentionGrant` (`crates/worldwake-core/src/contention.rs:43`) tracks expiry via `expires_at`, and its lifecycle is queue-state-mediated rather than carried by a discrete invalidation event. The resume path checks the existing contention-state read surface (`RuntimeBeliefView::facility_grant` for facility queues, plus the existing extraction-slot claimable/held reads) and fires the `ArtifactLegalEffectActive` / `OpportunityVisible` resume condition when the contested resource becomes available again. No new event tag is introduced.
...
S149 is plain GOAP/affordance search. Resumption re-enters the *existing* tactical search through the stored `GoalOffer` once the segment's resume condition holds; it must not register any HTN `MethodSchema` and must not encode goal-specific decomposition (the Non-Goals exclude method-decomposition resumption — that lives in S147). Live reassessment during S149PARPLASEG-005 and S149PARPLASEG-010 showed that the current `PlannedSkeletonStep` carrier is not executable by itself, so S149PARPLASEG-010 did not invent a parallel skeleton replay path. The typed terminals are search outcomes, not method contracts. No method-required goal is introduced, so no schema contract or fallback-invalidity argument is needed.
