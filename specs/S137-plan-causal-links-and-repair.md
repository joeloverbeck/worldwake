...
The codebase already classifies replanned plans into a 4-variant `RepairKind` (`AlternateTarget`, `AlternateRoute`, `AlternateMerchant`, `AlternateRecipe` at `crates/worldwake-core/src/decision_event_payload.rs:418-423`) and emits `EventTag::RepairApplied` when the new plan differs from the failed plan along a recognized axis (`classify_accepted_repair` at `crates/worldwake-ai/src/agent_tick/planning.rs:1452-1526`). This is *post-hoc* classification — the agent still pays the full-replan cost; the repair label is applied after the fact based on plan-shape diffing.
...
`Vec<CausalLink>` per the workspace-external-dependency rule (smallvec is not a workspace dependency). The cap is enforced at construction time by the planner emitter and validated at revalidation time. Note the distinction in prose: `invalidators: Vec<InvalidatorTag>` (core, discriminant-only) is the registration surface; `Invalidator` (ai-crate, payload-carrying at `plan_guard.rs:35-50`) is the breach-detection surface raised when revalidation finds a mismatch.
