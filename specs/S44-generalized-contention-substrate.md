...
- `worldwake-core` (new `ContentionQueue`, `ContentionPolicy`, `ContentionGrant` types; extend or generalize existing `FacilityUseQueue`)
...
- S42 is independent (can be parallelized).
- S43 is independent (can be parallelized).
...
5. **Phased rollout**: Start with the highest-value contention domains (corpse looting, item pickup) and extend incrementally. Don't try to cover every affordance in one spec.
...
This has the same API surface as `FacilityUseQueue` (enqueue, position_of, has_actor, remove_actor, promote_head, clear_grant, grant_expired) but is registered as a general-purpose component, not limited to workstation entities.
...
### 3. Migrate `FacilityUseQueue` to use `ContentionQueue`

Two migration strategies:

**(a) Thin wrapper**: `FacilityUseQueue` becomes a type alias or thin wrapper around `ContentionQueue` with facility-specific convenience methods. Existing callers are updated.

**(b) Replace in place**: Remove `FacilityUseQueue` entirely and use `ContentionQueue` + `ContentionPolicy` directly. Existing facility queue system reads the general-purpose components.

Option (b) is cleaner (no duplicate types) but requires updating all call sites. The implementer should choose based on migration effort. Both are architecturally valid.

### 4. Contention domains — Phase 1 targets

Attach `ContentionQueue` + `ContentionPolicy` to these entity classes:

| Entity | Exclusive Affordance | Current Resolution | New Resolution |
|--------|---------------------|-------------------|----------------|
| Corpse | Loot | Tick order (first action to start wins) | Queue/grant — one looter at a time, others wait or replan |
| Corpse | Bury | Tick order | Queue/grant — shared with loot queue |
| Unique items on ground | Pick up | Tick order | Race — first to start wins; others get structured StartFailed |
| Patients (wounded agents) | TreatWounds | Tick order | Queue/grant — one healer per patient at a time |
...
### 5. Contention-aware action validation (`worldwake-sim`)

Extend `action_validation.rs` to check contention state for queued domains:

- Before starting a loot/bury/treat action on a contention-managed entity, check if the actor holds the grant or can be enqueued.
...
- If the queue is full, return a structured "contention_rejected" result.

### 6. Contention-aware affordance generation (`worldwake-sim`/`worldwake-ai`)

`get_affordances()` should annotate affordances on contention-managed entities with queue state:
...
### 7. Contention system tick (`worldwake-systems`)
...
This reuses the same logic currently in `facility_queue.rs` but generalized.
...
`ContentionSystem` should run after domain action systems (so actions can complete and release grants) and before Perception (so contention state is observable in the same tick):

```
Needs → Production → Trade → Combat → FacilityQueue/Contention → Politics → Perception
```

If `FacilityUseQueue` is migrated to `ContentionQueue`, the existing FacilityQueue system slot becomes the Contention system slot.

## Migration Path

1. Add `ContentionQueue`, `ContentionPolicy`, `ContentionGrant`, `ContentionWaiter` to `worldwake-core`.
2. Implement queue operations (same API as `FacilityUseQueue`).
3. Add `contention_system()` to `worldwake-systems`.
4. Migrate `FacilityUseQueue` → `ContentionQueue` (or alias).
5. Attach `ContentionQueue` + `ContentionPolicy` to corpse entities and patient entities.
6. Add contention checks to loot, bury, and treat action validation.
7. Extend affordance generation with `ContentionStatus`.
8. Add perception of contention state.
9. Write golden tests.
10. Bump `SAVE_FORMAT_VERSION`.

## Verification

- `cargo test --workspace` passes — existing facility queue behavior unchanged.
...
- Save/load round-trip preserves `ContentionQueue` and `ContentionPolicy`.
