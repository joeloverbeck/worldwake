...
| FND-14 (World State Is Not Belief State) | The candidate emitter reads each candidate place's `SleepQualityProfile` through `GoalBeliefView::place_sleep_quality_profile` (belief-mediated). The action handler reads authoritative `SleepQualityProfile` at execution time (correct — actions execute against world state). |
