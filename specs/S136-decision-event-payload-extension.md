...
2. **No new computation on the success path; mechanical classification on the failure path.** The dimension (`rejection_dimension`) and the assumption list (`assumptions`) populate from state already computed during ranking and frame preparation. The failure-path `decisive_*` fields populate from the failed-claim/observation set already passed as a function input to each failure emission site (e.g., the `Discrepancy` payload, the violated expectation set, the contradicted-claim set). No new ranking pass, no new belief query.
...
4. **Failure-path `decisive_*` is not exhaustive.** The fields name the *load-bearing* facts on the failure decision — beliefs/records/observations whose absence or contradiction would have flipped the outcome — sourced from each failure site's existing function-input set. Promoting always-on decisive classification to the success path (`GoalCommitted`/`PlanAdopted`) is deliberately deferred to a follow-on spec because no such mechanical classifier exists today outside `DecisionTraceSink`.
5. **`rejection_dimension` is post-tiebreaker, not pre-rank.** The dimension recorded on each `RejectedAlternativeSummary` is the `RankedGoalComparisonOutcome::decisive_dimension` produced by `ranked_goal_ordering` (`ranking.rs:2389-2432`) — i.e., the dimension that ordered the rejected goal against the chosen one. Pre-rank-filtered goals (suppressed, infeasible at probe) continue to surface through their existing `GoalSuppressed`/`GoalOffered` events with the existing `rejection_reason: GoalRejectionReason` field, not via dimension.
...
| FND-15 (Knowledge Is Acquired Locally and Travels Physically) | `decisive_beliefs` and `decisive_records` (failure path) reference the per-agent provenance the agent acted on. Reconstructing the knowledge path remains traceable. |
...
- `RejectedAlternativeSummary.rejection_dimension` ← `RankedGoalComparisonOutcome::decisive_dimension` (already computed at `ranking.rs:2587`), converted to `RankedGoalComparisonDimensionTag`. Wired in `build_rejected_alternatives` at `planning.rs:931-1002`.
- `decisive_beliefs/records/world_observations` ← derived from the existing function inputs to each failure emission site:
...
1. **Information-path analysis.** No new information path — every reference S136 stores in the payload was already a function input to the emission site (rejection-comparison outcome at the success site; failed-claim/observation set at each failure site; populated frame assumptions post-D5 reorder). The path is now *recorded* rather than discarded after the tick.
...
- **Golden coverage**: new `golden_decision_payload.rs` with four scenarios:
  ...
  4. Source-expectation failure → assert `SourceExpectationFailurePayload.decisive_*` names the source-attribution input (no `assumptions` field — by D4).
