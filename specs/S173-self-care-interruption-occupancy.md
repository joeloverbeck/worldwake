...
`exclusive_facility_workstation_tag` (`facility_queue_actions.rs:152-172`) matches only on `ActionPayload::Harvest` and `ActionPayload::Craft`. It does not match on `wash`/`toilet` (which use `ActionPayload::None`), so neither variant flows through the existing `FacilityExclusive(WashBasin)` auto-promotion path today; both `SelfCareWash` and `SelfCareLatrine` are genuine net-new variants.
...
The CLI POV boundary already enforces this gating for remote facility state (S163); this spec confirms no new accessor on `GoalBeliefView` is required — `facility_wash_basin_state` (`belief_view.rs:495`) and `wash_basin_state` (`belief_view.rs:545, 1047`) already provide the FND-14A/14B-split surface from which `SelfCareOccupancy` can be read by the consumer that owns this composite query.

### D6. Candidate-emitter occupancy filter

`crates/worldwake-ai/src/candidate_generation.rs::wash_access_opportunities` (line 4744) currently filters wash candidates on `facility_wash_basin_state(*workstation).is_some_and(|state| state.clean_water_units > 0)` (line 4765-4767). This filter must be extended with a parallel occupancy check, conditioned on the FND-14B source-class table in D5: if the actor is the occupant or co-located with the facility, read `SelfCareOccupancy` from world state (FND-14A); if remote, gate on belief.
