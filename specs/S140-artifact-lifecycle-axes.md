...
2. `legal_effect` (handles `Expired` ← TTL, `Revoked` ← withdrawal/revocation transition events, `Fulfilled` ← bounty-fulfillment events from `commit_claim_bounty` at `artifact_actions.rs:1434`; source-event-backed `Suspended` / restoration handling is owned by tickets/S140ARTLIFAXE-007.md).
3. `credibility` (source-event-backed `Disputed` ← contradicting-testimony events and `Refuted` ← evidence-against events are owned by tickets/S140ARTLIFAXE-007.md; S140ARTLIFAXE-006 proves the lower lifecycle cascade from explicit credibility transition payloads).
...
- **Golden coverage**: new `golden_artifact_lifecycle.rs` with five scenarios:
  ...
  4. Suspended legal effect under jurisdiction conflict → expects `legal_effect: Active → Suspended`, restoration on resolution. S140ARTLIFAXE-006 proves the lower lifecycle transition/cascade with explicit transition payloads; tickets/S140ARTLIFAXE-007.md owns source-event-backed suspension/restoration.
  5. False rumor refuted via contradicting evidence → expects `credibility: Credible → Refuted`, `actionability: Actionable → Closed`, `existence: Exists` retained for audit. S140ARTLIFAXE-006 proves the lower lifecycle transition/cascade without depending on accusations being landed as artifacts; tickets/S140ARTLIFAXE-007.md owns replacing the explicit transition fixture with a source-event-backed refutation path where live evidence carriers allow it.
