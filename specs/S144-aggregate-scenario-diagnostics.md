...
Pure function. Single pass through each trace collection; deterministic sort + percentile computation; no I/O. `EventLog` exposes no whole-log iterator, so the aggregator walks the event log via `events_at_tick(tick)` across `tick_range` (and `events_by_tag(EventTag::QueueGrantPromoted)` etc. for the targeted coordination metrics). Performance-metric inputs are read from the existing `OpportunityCompilerLoad` carrier on `AgentDecisionTrace` (`crates/worldwake-ai/src/decision_trace.rs:99`, struct at `:889`) and `SearchExpansionSummary` on `PlanAttemptTrace` — no new carrier type is introduced. Test coverage proves identical reports across reruns.
...
- **Snapshot cache counters** — `PlanningSnapshot` (`crates/worldwake-ai/src/planning_snapshot.rs`) holds `BTreeMap`/`BTreeSet`/`DistanceMatrix` caches but does not currently count logical hit/miss/invalidation events. D8 adds read-only `u64` logical counters to those caches and surfaces them through the decision trace. This is a derived read-model addition (FND-27), not authoritative state, and changes no planning behavior.
- **Queue wait ticks** — read from event-log events tagged `QueueGrantPromoted` / `QueueGrantExpired` / `QueueHeadFailed` / `ContentionResolved` (the queue-grant `EventTag` variants in `crates/worldwake-core/src/event_tag.rs`). `ResourceExtractionQueues` is an ECS `Component`, not an event emitter — the *system* emits these tags. The aggregator computes wait distributions from event-pair latency.
